/**
 * Language information and locale data.
 * Migrated from apps/web/apps/common/main/lib/util/LanguageInfo.js
 */

export const localLanguageName: Record<number, readonly [string, string?, string?]> = {
  54: ["af", "Afrikaans"],
  ...
  5146: ["bs-Latn-BA", "Bosanski (Bosna i Hercegovina)", "Bosnian (Latin, Bosnia and Herzegovina)"],
  126: ["br", "Brezhoneg"],
  ...
  1152: ["ug-CN", "ئۇيغۇر يېزىقى (جۇڭخۇا خەلق جۇمھۇرىيىتى)‏", "Uighur (People's Republic of China)"],
  30787: ["uz-Cyrl", "Ўзбек"],
...
export function getLocalLanguageName(code: number): readonly [string, string?, string?] {
  return localLanguageName[code] || (["", code] as const)
}

export function getLocalLanguageDisplayName(
  code: number,
): { native: string; english: string } | null {
...
export function getLocalLanguageCode(name: string): string | null {
  if (name) {
...
export function getDefaultLanguageCode(name: string): number | undefined {
  const lowerName = name.toLowerCase()
  const defMap = defLanguages as unknown as Record<string, number>
  if (defMap[lowerName]) return defMap[lowerName]
  const code = getLocalLanguageCode(`${lowerName}-${name.toUpperCase()}`)
  return code ? Number(code) : undefined
}

export function getLanguages(): typeof localLanguageName {
...
let regionalData: RegionalDataItem[] | null = null

export function getRegionalData(): RegionalDataItem[] {
