...
;(async () => {
  ...
  const outDir =
    process.env.OUT_DIR || "/home/weiss/git/World-Office/server/.sisyphus/evidence/screenshots"
  ...
  try {
    ...
    if (collabEl) {
      ...
      await page.screenshot({
        path: `${outDir}/collaboration-status.png`,
        clip: { x: 1180, y: 0, width: 220, height: 60 },
      })
    ...
    if (holderEl) {
      ...
      await page.screenshot({
        path: `${outDir}/remote-cursor-demo.png`,
        clip: { x: 0, y: 60, width: 1400, height: 500 },
      })
