...
test.describe("WOPI Integration @headed", () => {
  ...
  test("full WOPI chain: upload → app/open → CheckFileInfo → editor loads", async ({ page }) => {
    ...
    console.log(
      `✅ WOPI params parsed (file=${fileIdInWopi.substring(0, 16)}..., token=${wopiToken.substring(0, 20)}...)`,
    )
    ...
    console.log(
      `✅ Editor rendering (canvas=${state.hasCanvas}, error=${state.isError}, title=${state.title})`,
    )
