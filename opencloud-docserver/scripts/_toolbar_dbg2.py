...
def _serve(port):
    ...
    cfg = Config(port=port, host="127.0.0.1",
                 database=f"/tmp/toolbar-dbg2-{port}/d.db",
                 content_dir=f"/tmp/toolbar-dbg2-{port}/doc", public_url="http://x")
...
with sync_playwright() as p:
    ...
    def sel():
        return page.evaluate("() => { const s=window.getSelection(); return s.rangeCount? s.toString():''; }")

    # Existing BOLD button with a selection
    page.click('#editor p:has-text("Debug line one")')
    page.keyboard.press("Shift+Home")
    print("sel-before-bold-click:", repr(sel()), flush=True)
    page.click('button[data-cmd="bold"]')
    page.wait_for_timeout(200)
    print("after-bold-click:", editor.evaluate("(el)=>el.innerHTML"), flush=True)

    # Test: mouse-drag selection then click bold (more realistic)
    page.click('#editor p:has-text("Debug line two")')
    page.keyboard.press("Shift+Home")
    print("sel-before-bold-click2:", repr(sel()), flush=True)
    # dispatch click without moving mouse focus via DOM (bypass playright mouse)
    page.evaluate("() => { document.querySelector('button[data-cmd=\"bold\"]').click(); }")
    page.wait_for_timeout(200)
    print("after-bold-domclick:", editor.evaluate("(el)=>el.innerHTML"), flush=True)
