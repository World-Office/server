"""E2E — images through production OpenCloud (OCIS client mode).
...
on the DOCX image path the editor UI offers (insert-image dialog ->
self-contained data-URI <img> at the caret, plus pictures that already
live in a DOCX as w:drawing/a:blip parts):

    seed a DOCX with or without an embedded picture on the OpenCloud host
      -> OpenCloud launches /editor with access_token + WOPISrc  (WOPI handshake)
      -> GET  /api/documents/{id}/html                           (DOCX -> HTML)
      ...
      -> POST /api/documents/{id}/save                           (HTML -> DOCX)
      -> docserver PUTs the bytes back to the remote host          (PutFile)
      -> GET  /api/documents/{id}/html again                      (reload)
      -> re-read the DOCX stored on the remote host and verify its
         word/media/ member bytes match the uploaded file and
         word/document.xml carries a w:drawing/a:blip picture.
...
DOCX converter (which embeds data-URI images as w:drawing picture
...
the upload dialog — or already present in a seeded document — survives
every hop of the production path, and the stored bytes are a real DOCX
whose package holds the very same picture bytes.

Marker emitted when the full production-path DOCX image persistence
check passes:
...
# treated as successful until the marker is found in the stored DOCX.
...
def _build_docx(*lines: str) -> bytes:
    """Build a DOCX with one plain paragraph per line."""
    doc = Document()
    for line in lines:
...
def _build_docx_with_picture(png: bytes, caption: str = "imaged ") -> bytes:
    """Build a DOCX whose paragraph embeds a w:drawing inline picture."""
    doc = Document()
    p = doc.add_paragraph()
    p.add_run(caption)
    run = p.add_run()
...
# ----------------------------------------------------------------------
# The E2E tests — images through production OpenCloud (DOCX path)
# ----------------------------------------------------------------------

def test_docx_image_upload_persists_through_production_opencloud(stack):
    """The full production roundtrip for a picture inserted via the upload
    dialog into a DOCX: OpenCloud launch -> HTML -> craft the self-contained
    data-URI <img> the dialog inserts -> save -> PutFile -> reload. Emits
    the IMAGE-PERSISTENCE marker only when the DOCX stored back on the
    OpenCloud host carries the very same picture bytes (word/media member +
    w:drawing/a:blip in document.xml)."""
    stack.host.seed("img-docx", "report.docx", _build_docx("Entwurf Bericht"))
    stack.launch_editor("img-docx", user="alice")

    # OpenCloud serves the DOCX as HTML for the editor.
    html = stack.load_html("img-docx")
    assert "Entwurf Bericht" in html
    assert _img_srcs(html) == [], "no images yet"

    # The user picks logo.png in the upload dialog; the dialog turns it into
    # a self-contained data URI and inserts <img> at the caret (the same
    # markup document.execCommand("insertHTML", ...) produces), then types a
    # caption next to it and saves.
    png = _png_bytes(3, 2)
    uploaded = f'<p>{MARKER}: Foto <img src="{_data_uri(png)}" alt="logo.png"/></p>'
    stack.save_html("img-docx", html + "\n" + uploaded)
    ...
    reloaded = stack.load_html("img-docx")
    ...
    # The stored bytes are a real DOCX: document.xml carries the picture as
    # a w:drawing/a:blip and the package embeds the original bytes.
    stored = stack.remote_doc("img-docx")
    content = _docx_content_xml(stored)
    assert "w:drawing" in content and "a:blip" in content, content
    media = _docx_media_bytes(stored)
    assert len(media) == 1
    assert media[0] == png, "stored DOCX picture bytes must match the upload"
    assert MARKER in _docx_text(stored)

    print("IMAGE-PERSISTENCE: OK")


def test_docx_image_upload_into_empty_document_first_save(stack):
    """Starting from an empty DOCX, a picture uploaded as the document's only
    content lands on the remote host as a real DOCX with the image embedded."""
    stack.host.seed("img-blank", "pic.docx", _build_docx())
    ...
    content = _docx_content_xml(stored)
    assert "w:drawing" in content and "a:blip" in content, content
    assert _docx_media_bytes(stored) == [png]
...
def test_docx_image_upload_keeps_alt_and_surrounding_text(stack):
    """The uploaded picture's alt text becomes the drawing's wp:docPr descr
    and is re-exported as an alt attribute on reload, with the surrounding
    text order preserved."""
    stack.host.seed("img-alt", "alt.docx", _build_docx("Intro line"))
    stack.launch_editor("img-alt", user="alice")

    png = _png_bytes(2, 2)
    edited = (
        stack.load_html("img-alt")
        + f'\n<p>before <img src="{_data_uri(png)}" alt="Kapitel Foto"/> after</p>'
        + f"\n<p>{MARKER}-tail</p>"
    )
    ...
    content = _docx_content_xml(stored)
    assert "Kapitel Foto" in content, "alt must become wp:docPr descr in document.xml"
    assert _docx_media_bytes(stored) == [png]

    reloaded = stack.load_html("img-alt")
    assert "before " in reloaded and " after" in reloaded
    assert f"{MARKER}-tail" in reloaded
    assert 'alt="Kapitel Foto"' in reloaded
    srcs = _img_srcs(reloaded)
    assert len(srcs) == 1
    assert _decode_data_uri(srcs[0]) == png


def test_picture_in_seeded_docx_survives_edit_and_save(stack):
    """A document that already contains a picture (w:drawing) loads into the
    editor as a data-URI <img>; editing next to it and saving keeps the
    picture bytes verbatim on the remote host."""
    png = _png_bytes(5, 4)
    stack.host.seed("img-seeded", "seeded.docx", _build_docx_with_picture(png))
    ...
    assert _docx_media_bytes(stored) == [png], "picture bytes must survive the edit"
    content = _docx_content_xml(stored)
    assert "w:drawing" in content and "a:blip" in content
    assert f"{MARKER}-caption" in _docx_text(stored)
...
def test_docx_image_upload_survives_close_and_reopen(stack):
    ...
    stack.host.seed("img-reopen", "reopen.docx", _build_docx("Open"))
...
def test_unsafe_image_urls_are_dropped_on_save_docx(stack):
    """The upload path must not weaken sanitization on the DOCX route either:
    an <img> whose src is a script scheme (javascript:) is dropped before
    conversion, so neither an executable URL nor a stray picture reaches the
    stored bytes."""
    from src.editor.sanitize import sanitize_html

    stack.host.seed("img-safe", "safe.docx", _build_docx("Clean"))
    ...
    # sanitizer keeps no script-scheme src, and the save path must never
    # embed a picture from it.
    ...
    assert _docx_media_bytes(stored) == [], "no picture may be embedded from a script URL"
    assert "alert" not in _docx_content_xml(stored)
