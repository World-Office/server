...
import type { FlowchartNode, FlowchartEdge, ArrowheadType } from "../types/visio"
    ...
    <div className="fc-props-section">
      ...
      <div className="fc-props-row">
        ...
        <label className="fc-props-label">
          ...
          <select
            ...
            onChange={(e) => updateEdge({ strokeStyle: e.target.value as "solid" | "dashed" | "dotted" })}
          >
            {STROKE_STYLES.map((s) => (
      ...
      <label className="fc-props-label">
        ...
        <select
          ...
          onChange={(e) => updateEdge({ arrowheadType: e.target.value as ArrowheadType })}
        >
...
export const PropertiesPanel = observer(function PropertiesPanel(): JSX.Element | null {
  const store = flowchartStore
  const selectedNodeId = store.selectedNodeIds.length === 1 ? store.selectedNodeIds[0] : null
  const selectedEdgeId = store.selectedEdgeIds.length === 1 ? store.selectedEdgeIds[0] : null
  const node = selectedNodeId ? store.document.nodes.find((n) => n.id === selectedNodeId) : null
  const edge = selectedEdgeId ? store.document.edges.find((e) => e.id === selectedEdgeId) : null
