...
import type { FlowchartDocument, FlowchartEdge, FlowchartNode } from "../types/visio"
...
function getEdgeEndpoint(node: FlowchartNode, anchor: string | undefined): Point {
...
function eventToSVGPoint(e: React.MouseEvent | MouseEvent, svg: SVGSVGElement): Point {
...
const FlowchartNodeRenderer = observer(function FlowchartNodeRenderer({
  ...
  return (
    ...
    >
      ...
      {/* Label */}
      <text className={styles.nodeLabel} x={x + w / 2} y={y + h / 2} fontSize={fontSize || 14}>
        {label}
      </text>

      {/* Inline editing overlay */}
      {isEditing && (
        <foreignObject className={styles.editForeignObject} x={x} y={y} width={w} height={h}>
          <input
  ...
  const handleSVGMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (isPanning) {
        ...
        store.setCanvasOffset(panOffsetStartRef.current.x - dx, panOffsetStartRef.current.y - dy)
        return
      }
      ...
      if (store.isResizing && store.resizeNodeId && store.resizeStartNode) {
        ...
        let nh = start.height
        const snap = (v: number) =>
          store.snapToGridEnabled ? Math.round(v / store.gridSize) * store.gridSize : v
      ...
      if (isRubberBanding && rubberBandRect) {
        ...
        setRubberBandRect((prev) => (prev ? { ...prev, x2: svgPoint.x, y2: svgPoint.y } : prev))
      }
    },
  ...
  return (
    <div ref={containerRef} className={styles.container}>
      ...
      >
        <defs>
          {gs > 0 && (
            <pattern id="fc-grid" width={gs} height={gs} patternUnits="userSpaceOnUse">
        ...
        {doc.nodes.map((node) => (
          <g key={node.id}>
            <FlowchartNodeRenderer
              ...
              isConnectTarget={store.connectSourceId !== null && store.connectSourceId !== node.id}
              ...
              onEditKeyDown={handleEditKeyDown}
            />
            {store.selectedNodeIds.includes(node.id) && <ResizeHandles node={node} />}
          </g>
        ))}
      </svg>
