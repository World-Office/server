...
export class FlowchartStore {
  ...
  copySelection(): void {
    const selectedNodes = this.document.nodes.filter((n) => this.selectedNodeIds.includes(n.id))
  ...
  alignLeft(): void {
    ...
    const minX = Math.min(
      ...this.selectedNodeIds.map(
        (id) => this.document.nodes.find((n) => n.id === id)?.x ?? Number.POSITIVE_INFINITY,
      ),
    )
  ...
  alignTop(): void {
    ...
    const minY = Math.min(
      ...this.selectedNodeIds.map(
        (id) => this.document.nodes.find((n) => n.id === id)?.y ?? Number.POSITIVE_INFINITY,
      ),
    )
    for (const id of this.selectedNodeIds) {
      const n = this.document.nodes.find((nd) => nd.id === id)
