export const CONVERSION_API_URL: string =
  (typeof window !== "undefined" &&
    ((window as unknown as Record<string, unknown>).__CONVERSION_API_URL as string)) ||
...
export async function exportToPPTX(): Promise<void> {
  ...
  try {
    ...
    const fallback = confirm("PPTX conversion is not available. Download as JSON instead?")
    if (fallback) {
