...
const chartTypes: { type: ChartType; label: string; icon: JSX.Element }[] = [
  ...
  },
  {
    ...
    icon: (
      <svg width="32" height="24" viewBox="0 0 32 24">
        <polyline points="3,18 10,12 17,6 24,10 29,15" fill="none" stroke="#4472C4" strokeWidth="2" />
...
function addChart(type: ChartType) {
  ...
  const sampleData: Record<ChartType, { title: string; labels: string[]; series: { name: string; values: number[] }[] }> = {
    bar: {
...
const ObservedChartTypePicker = observer(function ObservedChartTypePicker() {
  ...
  return (
    <div ref={ref} className="prese-chart-type-picker" style={{ position: "relative", display: "inline-block" }}>
      <button
        type="button"
