...
function renderChartSvg(chart: ChartData, width: number, height: number): JSX.Element[] {
  ...
  if (chart.title) {
    elements.push(
      <text key="cht" x={width / 2} y={16} textAnchor="middle" fontSize={14} fontWeight="bold" fill="#333">
        {chart.title}
      </text>,
    )
  }
    ...
    chart.labels.forEach((label, li) => {
      chart.series.forEach((series, si) => {
        ...
        if (isBar) {
          ...
          elements.push(<rect key={`bar-${li}-${si}`} x={pad.left} y={y} width={barW} height={barH} fill={color} rx={1} />)
          if (si === 0) {
            elements.push(<text key={`lb-${li}`} x={pad.left - 4} y={y + barH / 2 + 4} textAnchor="end" fontSize={10} fill="#666">{label}</text>)
          }
        } else {
          ...
          elements.push(<rect key={`col-${li}-${si}`} x={x} y={y} width={barW} height={barH} fill={color} rx={1} />)
          if (si === 0) {
            elements.push(<text key={`lb-${li}`} x={x + barW / 2} y={pad.top + chartH + 14} textAnchor="middle" fontSize={10} fill="#666">{label}</text>)
          }
        }
      })
    })
  ...
  if (chart.type === "line") {
    ...
    chart.series.forEach((series, si) => {
      ...
      elements.push(<path key={`line-${si}`} d={d} stroke={color} strokeWidth={2} fill="none" />)
      pts.forEach((p, pi) => {
        elements.push(<circle key={`pt-${si}-${pi}`} cx={p.x} cy={p.y} r={3} fill={color} />)
      })
    })
    chart.labels.forEach((label, li) => {
      const x = pad.left + (li / Math.max(pointCount - 1, 1)) * chartW
      elements.push(<text key={`lb-${li}`} x={x} y={pad.top + chartH + 14} textAnchor="middle" fontSize={10} fill="#666">{label}</text>)
    })
  }
  ...
  if (chart.type === "pie" || chart.type === "doughnut") {
    ...
    chart.series.forEach((series, si) => {
      series.values.forEach((val, vi) => {
        ...
        elements.push(<path key={`pie-${si}-${vi}`} d={d} fill={color} stroke="#fff" strokeWidth={1} />)
        if (sliceAngle > 0.3) {
          const labelAngle = currentAngle + sliceAngle / 2
          const lr = radius * 0.7
          elements.push(
            <text key={`pv-${si}-${vi}`} x={cx + lr * Math.cos(labelAngle)} y={cy + lr * Math.sin(labelAngle)}
              textAnchor="middle" dominantBaseline="central" fontSize={11} fill="#fff" fontWeight="bold">
...
function renderTableSvg(table: TableData, width: number, height: number): JSX.Element[] {
  ...
  for (let ri = 0; ri < numRows; ri++) {
    for (let ci = 0; ci < numCols; ci++) {
      ...
      elements.push(
        <rect key={`tbg-${ri}-${ci}`} x={x} y={y} width={colWidth} height={rowHeight}
          fill={isHeader ? headerBg : "white"} stroke="#ccc" strokeWidth={0.5} />,
      )
      elements.push(
        <text key={`ttxt-${ri}-${ci}`} x={x + colWidth / 2} y={y + rowHeight / 2}
          textAnchor="middle" dominantBaseline="central" fontSize={11}
          fill={isHeader ? headerFg : "#333"} fontWeight={isHeader ? "bold" : "normal"}>
...
function renderConnectorSvg(connector: ConnectorData, width: number, height: number, stroke: string, strokeWidth: number): JSX.Element {
  ...
  const markerStart = connector.hasStartArrow ? `url(#${markerId}-start)` : undefined
  ...
  return (
    <svg key="conn-svg" width={width} height={height} style={{ overflow: "visible", position: "absolute", top: 0, left: 0, pointerEvents: "none" }}>
      <defs>
...
function shadowToFilter(shadow: ShadowEffect, id: string): JSX.Element {
  ...
  return (
    <filter id={filterId} x="-20%" y="-20%" width="140%" height="140%">
      <feDropShadow dx={shadow.dx / 100} dy={shadow.dy / 100} stdDeviation={blur}
        floodColor={shadow.color || "#000"} floodOpacity={shadow.opacity || 0.5} />
    </filter>
  )
}

function renderPresenterShape(shape: ShapeData, defs: JSX.Element[]): JSX.Element | null {
  ...
  const fillValue = hasGradient ? `url(#pgrad-${shape.id})` : shape.fillColor || "transparent"
  ...
  if (shape.chart) {
    ...
    return (
      <div key={shape.id} style={wrapperStyle}>
        <svg width={shape.width} height={shape.height}>{chartSvg}</svg>
      </div>
    )
  }
  ...
  if (shape.table) {
    ...
    return (
      <div key={shape.id} style={wrapperStyle}>
        <svg width={shape.width} height={shape.height}>{tableSvg}</svg>
      </div>
    )
  }
  ...
  switch (shape.type) {
      ...
      return (
        <div key={shape.id} style={wrapperStyle}>
          <svg width={shape.width} height={shape.height}>
            <rect x={0} y={0} width={shape.width} height={shape.height} fill={fillValue}
              stroke={strokeColor} strokeWidth={strokeW} filter={filterVal} rx={0} />
            {shape.text && <text x={shape.width / 2} y={shape.height / 2} textAnchor="middle" dominantBaseline="central"
              fill={shape.fontColor || "#333"} fontSize={shape.fontSize || 14}>{shape.text}</text>}
          </svg>
        </div>
      )
    case "roundedRect":
      return (
        <div key={shape.id} style={wrapperStyle}>
          <svg width={shape.width} height={shape.height}>
            <rect x={0} y={0} width={shape.width} height={shape.height} fill={fillValue}
              stroke={strokeColor} strokeWidth={strokeW} filter={filterVal} rx={8} />
            {shape.text && <text x={shape.width / 2} y={shape.height / 2} textAnchor="middle" dominantBaseline="central"
              fill={shape.fontColor || "#333"} fontSize={shape.fontSize || 14}>{shape.text}</text>}
          </svg>
        </div>
        ...
        <div key={shape.id} style={wrapperStyle}>
          <svg width={shape.width} height={shape.height}>
            <ellipse cx={shape.width / 2} cy={shape.height / 2} rx={shape.width / 2} ry={shape.height / 2}
              fill={fillValue} stroke={strokeColor} strokeWidth={strokeW} filter={filterVal} />
            {shape.text && <text x={shape.width / 2} y={shape.height / 2} textAnchor="middle" dominantBaseline="central"
              fill={shape.fontColor || "#333"} fontSize={shape.fontSize || 14}>{shape.text}</text>}
          </svg>
        </div>
    ...
    case "line":
      return (
        <div key={shape.id} style={wrapperStyle}>
          <svg width={shape.width} height={shape.height}>
            <line x1={0} y1={0} x2={shape.width} y2={shape.height}
              stroke={strokeColor} strokeWidth={strokeW || 2} filter={filterVal} />
          </svg>
        </div>
      )
    case "arrow":
      return (
        <div key={shape.id} style={wrapperStyle}>
          <svg width={shape.width} height={shape.height}>
            <defs>
              <marker id={`parrow-${shape.id}`} markerWidth={10} markerHeight={10} refX={9} refY={3} orient="auto">
                <path d="M0,0 L10,3 L0,6" fill={strokeColor} />
              </marker>
            </defs>
            <line x1={0} y1={shape.height / 2} x2={shape.width - 5} y2={shape.height / 2}
              stroke={strokeColor} strokeWidth={strokeW || 2} markerEnd={`url(#parrow-${shape.id})`} />
          </svg>
        </div>
      )
    case "connector":
      if (shape.connector) {
        return (
          <div key={shape.id} style={wrapperStyle}>
            {renderConnectorSvg(shape.connector, shape.width, shape.height, strokeColor, strokeW || 2)}
          </div>
        )
      }
      return null
...
const ObservedSlidePresenter = observer(function ObservedSlidePresenter(): JSX.Element {
  const { isPresenting, slides, presentStep, endPresentation, nextSlide, prevSlide } =
    presentationStore
  ...
  const nextSlideData = presentStep < slides.length - 1 ? slides[presentStep + 1] : null
  ...
  const shapeElements = slide.shapes?.map((shape) => renderPresenterShape(shape, svgDefs))
  ...
  return (
    ...
    >
      <div className="prese-presenter-main">
        <div className="prese-presenter-slide" style={{ position: "relative", overflow: "hidden", isolation: "isolate" }}>
          ...
          <div style={{ position: "relative", zIndex: 0 }}>
            ...
            {slide.layout === "content" && (
              <>
                ...
                {(!slide.shapes || slide.shapes.length === 0) && (
                  <div className="prese-presenter-slide-content" style={{ color: "#999", fontStyle: "italic" }}>
                    No content
                  </div>
                )}
              </>
            ...
            {slide.layout === "blank" && slide.title && (
              <div className="prese-presenter-slide-title" style={{ fontSize: "1.5rem", color: "#999", fontStyle: "italic", marginBottom: 0 }}>
                {slide.title}
              </div>
            )}
            ...
            {slide.notes && (
              <div className="prese-presenter-notes-panel">
                ...
                <div className="prese-presenter-notes-text">{slide.notes}</div>
              </div>
            )}
          </div>
      ...
      {nextSlideData && (
        <div className="prese-presenter-next">
          ...
          <div className="prese-presenter-next-slide">
            <div className="prese-presenter-next-title">{nextSlideData.title || "Untitled"}</div>
            {nextSlideData.notes && (
              <div className="prese-presenter-next-notes">{nextSlideData.notes}</div>
            )}
          </div>
        </div>
      )}
      ...
      <div className="prese-presenter-controls">
        ...
        <button className="prese-presenter-btn prese-presenter-btn-esc" onClick={endPresentation}>
          ✕ Exit (Esc)
        </button>
      </div>
    </div>
