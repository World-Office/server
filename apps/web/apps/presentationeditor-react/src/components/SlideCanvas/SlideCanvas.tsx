...
const ObservedSlideCanvas = observer(
	function ObservedSlideCanvas(): JSX.Element {
		...
		const onDragStart = useCallback((e: React.MouseEvent, shapeId: string) => {
			...
			const shapeIds = selectedSet.has(shapeId)
		...
		const onRotateStart = useCallback(
			(e: React.MouseEvent, shapeId: string) => {
				const shape = presentationStore.slides[
					presentationStore.currentSlide
				]?.shapes?.find((s) => s.id === shapeId);
				if (!shape) return;
				const slideEl = e.currentTarget.closest(
					'[class*="prese-canvas-slide"]',
				) as HTMLElement | null;
				if (!slideEl) return;
		...
		const onResizeStartCB = useCallback(
			(e: React.MouseEvent, shapeId: string, handle: string) => {
				const shape = presentationStore.slides[
					presentationStore.currentSlide
				]?.shapes?.find((s) => s.id === shapeId);
				if (!shape) return;
		...
		const handleMouseMove = useCallback((e: MouseEvent) => {
			...
			if (resizeRef.current) {
				const { shapeId, handle, startX, startY, origX, origY, origW, origH } =
					resizeRef.current;
				const dx = (e.clientX - startX) / (presentationStore.zoomLevel / 100);
				const dy = (e.clientY - startY) / (presentationStore.zoomLevel / 100);
				let newX = origX;
			...
			<div className="prese-canvas-container">
				<div
					...
					style={{
						...
						animationDuration: previewAnim
							? `${previewAnim.duration}s`
							: undefined,
