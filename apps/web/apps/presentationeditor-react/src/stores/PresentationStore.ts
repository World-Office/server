...
export class PresentationStore {
	...
	copyShape(): void {
		...
		this.clipboardShapes = this.selectedShapeIds
			...
			.map((s) => ({
				...s,
				id: `clipboard-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
			}));
	}
	...
	updateShape(
		...
		this.notifyCollaboration("shape_modify", {
			slide_index: slideIndex,
			shape_id: shapeId,
			properties: updates as Record<string, unknown>,
		});
	}

	private applyShapeUpdates(
		slideIndex: number,
		shapeId: string,
		updates: Partial<ShapeData>,
	): void {
		...
		const dw =
			typeof updates.width === "number" ? updates.width - shape.width : 0;
		const dh =
			typeof updates.height === "number" ? updates.height - shape.height : 0;
	...
	removeShape(slideIndex: number, shapeId: string): void {
		...
		if (slide?.shapes) {
			...
			this.notifyCollaboration("shape_delete", {
				slide_index: slideIndex,
				shape_id: shapeId,
			});
		}
	...
	moveShape(slideIndex: number, shapeId: string, x: number, y: number): void {
		...
		if (slide?.shapes) {
			...
			if (shape) {
				...
				this.notifyCollaboration("shape_move", {
					slide_index: slideIndex,
					shape_id: shapeId,
					x,
					y,
		...
		for (const shapeId of shapeIds) {
			...
			if (shape.groupId) {
				for (const member of slide.shapes) {
					if (
						member.id !== shapeId &&
						member.groupId === shape.groupId &&
						!movedIds.has(member.id)
					) {
						member.x += moveX;
						member.y += moveY;
	...
	bringForward(slideIndex: number, shapeId: string): void {
		...
		if (idx < slide.shapes.length - 1) {
			const a = slide.shapes[idx];
			const b = slide.shapes[idx + 1];
			const tmp = a.zIndex;
			a.zIndex = b.zIndex;
			b.zIndex = tmp;
			slide.shapes[idx] = b;
			slide.shapes[idx + 1] = a;
		}
	}
	...
	private applyZOrderToSelected(fn: (idx: number) => void): void {
		...
		const sorted = [...this.selectedShapeIds].sort((a, b) => {
			return (
				slide.shapes.findIndex((s) => s.id === a) -
				slide.shapes.findIndex((s) => s.id === b)
			);
		});
	...
	private onMutation:
		...
		| null = null;
	private onCursorMove: ((page: number, x: number, y: number) => void) | null =
		null;

	registerMutationCallback(
		cb: (action: string, data: Record<string, unknown>) => void,
	): void {
		this.onMutation = cb;
	}

	registerCursorSendCallback(
		cb: (page: number, x: number, y: number) => void,
	): void {
		this.onCursorMove = cb;
	}

	private notifyCollaboration(
		action: string,
	...
	getEffectiveTransition(index: number): {
		...
		return {
			...
			soundEnabled:
				slide?.transitionSoundEnabled ?? this.transitionSoundEnabled,
	...
	setAnimationTarget(slideIndex: number, animId: string, target: string): void {
		this.pushSnapshot();
		const anim = this.slides[slideIndex]?.animations?.find(
			(a) => a.id === animId,
		);
	...
	groupSelected(): void {
		const slide = this.slides[this.currentSlide];
		if (!slide?.shapes) return;
		const selected = slide.shapes.filter((s) =>
	...
	addImageToSlide(slideIndex: number, file: File): void {
		...
		reader.onload = () => {
			...
			const centerY = Math.round(
				(960 / (this.slideSize === "widescreen" ? 16 / 9 : 4 / 3) - 200) / 2,
			);
			const newShape: ShapeData = {
				id: `image-${Date.now()}`,
				type: "image",
			...
			this.notifyCollaboration("shape_add", {
				slide_index: slideIndex,
				shape: newShape,
			});
		};
	...
	reorderSlides(fromIndex: number, toIndex: number): void {
		...
		this.notifyCollaboration("slide_reorder", {
		...
		switch (action) {
			case "shape_add": {
				...
				if (typeof slideIndex === "number" && this.slides[slideIndex]) {
					if (!this.slides[slideIndex].shapes)
						this.slides[slideIndex].shapes = [];
			...
			case "shape_delete": {
				...
				if (typeof slideIndex === "number" && this.slides[slideIndex]?.shapes) {
					this.slides[slideIndex].shapes = this.slides[
						slideIndex
			...
			case "shape_modify": {
				...
				if (typeof slideIndex === "number" && this.slides[slideIndex]?.shapes) {
					const shape = this.slides[slideIndex].shapes.find(
						(s) => s.id === sid,
					);
					if (shape) {
