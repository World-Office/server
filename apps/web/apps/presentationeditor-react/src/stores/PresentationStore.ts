...
import { convertPptxToHtml, convertPptxToWoPresentation } from "../lib/conversion"
...
export class PresentationStore {
    ...
    if (this.selectedShapeIds.length === 1) {
      ...
      const minX = Math.min(
        ...this.selectedShapeIds.map(
          (id) => slide.shapes.find((s) => s.id === id)?.x ?? Number.POSITIVE_INFINITY,
        ),
      )
  ...
  alignTop(): void {
    ...
    if (this.selectedShapeIds.length === 1) {
      ...
      const minY = Math.min(
        ...this.selectedShapeIds.map(
          (id) => slide.shapes.find((s) => s.id === id)?.y ?? Number.POSITIVE_INFINITY,
        ),
      )
      for (const id of this.selectedShapeIds) {
        const shape = slide.shapes.find((s) => s.id === id)
  ...
  bringToFrontSelected(): void {
    this.applyZOrderToSelected(() => {
      ...
      const idx = firstId ? slide.shapes.findIndex((s) => s.id === firstId) : -1
    ...
    switch (action) {
      ...
      case "shape_delete": {
        ...
        if (typeof slideIndex === "number" && this.slides[slideIndex]?.shapes) {
          this.slides[slideIndex].shapes = this.slides[slideIndex].shapes.filter(
            (s) => s.id !== shapeId,
          )
        }
        break
