...
export const TrackInsertMark = Mark.create({
	...
	addAttributes() {
		return {
			author: { default: null, parseHTML: (el) => (el as HTMLElement).getAttribute("data-author"), renderHTML: (attrs) => attrs.author ? { "data-author": attrs.author as string } : {} },
			timestamp: { default: null, parseHTML: (el) => (el as HTMLElement).getAttribute("data-timestamp"), renderHTML: (attrs) => attrs.timestamp ? { "data-timestamp": attrs.timestamp as string } : {} },
			userId: { default: null, parseHTML: (el) => (el as HTMLElement).getAttribute("data-user-id"), renderHTML: (attrs) => attrs.userId ? { "data-user-id": attrs.userId as string } : {} },
		}
	},
	parseHTML() { return [{ tag: "ins[data-author]" }] },
	renderHTML({ HTMLAttributes }) { return ["ins", { ...HTMLAttributes, class: "track-insert" }, 0] },
})
...
export const TrackDeleteMark = Mark.create({
	...
	parseHTML() { return [{ tag: "del[data-author]" }] },
	renderHTML({ HTMLAttributes }) { return ["del", { ...HTMLAttributes, class: "track-delete" }, 0] },
})
...
export function acceptChange(editor: Editor, pos: number): boolean {
	...
	for (const mark of marks) {
		...
		if (mark.type.name === "trackDelete") {
			editor.chain().focus().unsetMark("trackDelete").deleteRange({ from: pos, to: pos + 1 }).run()
			return true
		}
	}
	return false
...
export function rejectChange(editor: Editor, pos: number): boolean {
	...
	for (const mark of marks) {
		if (mark.type.name === "trackInsert") {
			editor.chain().focus().unsetMark("trackInsert").deleteRange({ from: pos, to: pos + 1 }).run()
			return true
		}
		if (mark.type.name === "trackDelete") {
