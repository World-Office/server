/** Image settings panel for spreadsheet editor. */
import type { JSX } from "react";
interface Props {
	visible: boolean;
}
export function ImageSettingsPanel({ visible }: Props): JSX.Element | null {
	...
	return (
		<div className="se-properties-panel" style={p.panel}>
			<div style={p.header}>Image Settings</div>
			<div style={p.body}>
				<div style={p.sec}>
					<div style={p.label}>Size</div>
					<div style={{ display: "flex", gap: 8 }}>
						<div style={{ flex: 1 }}>
							<label style={p.smLabel}>
								Width
								<input
									type="number"
									defaultValue={200}
					...
					<label style={p.chk}>
						<input
							...
							defaultChecked
							onChange={(e) =>
								cmd("imageLockAspect", e.target.checked ? "true" : "false")
							}
						/>
						Lock aspect ratio
					</label>
...
const p: Record<string, React.CSSProperties> = {
	...
	smLabel: { display: "block", fontSize: 11, color: "#888", marginBottom: 2 },
	inp: {
		width: "100%",
		padding: "4px 8px",
