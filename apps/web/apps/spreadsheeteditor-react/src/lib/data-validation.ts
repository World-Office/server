...
export function getValidationForCell(cellRef: string): DataValidationRule | undefined {
	return appliedValidations.get(cellRef)
}

export function validateCellValue(cellRef: string, value: string | number | null): string | null {
	...
	switch (rule.type) {
		case "list":
			if (rule.listItems && !rule.listItems.includes(String(value))) {
				return rule.errorMessage ?? `Value must be one of: ${rule.listItems.join(", ")}`
			}
			break
		case "numberRange": {
			const num = Number(value)
			if (Number.isNaN(num)) return rule.errorMessage ?? "Value must be a number"
			if (rule.min !== undefined && num < rule.min) return rule.errorMessage ?? `Value must be ≥ ${rule.min}`
			if (rule.max !== undefined && num > rule.max) return rule.errorMessage ?? `Value must be ≤ ${rule.max}`
			break
		}
		case "textLength": {
			const len = String(value ?? "").length
			if (rule.min !== undefined && len < rule.min) return rule.errorMessage ?? `Text must be ≥ ${rule.min} characters`
			if (rule.max !== undefined && len > rule.max) return rule.errorMessage ?? `Text must be ≤ ${rule.max} characters`
			break
		}
	}
	return null
