...
;((window) => {
  ...
  window.define = (id, deps, factory) => {
    ...
    var req = (childId) => window.require(id, childId)
    ...
    window.require.modules[id] = {
      ...
      factory: function () {
        ...
        if (returnExports) this.exports = returnExports
        return this
      },
    }
  }
...
ace.define("ace/range", ["require", "exports", "module"], (require, exports, module) => {
  var comparePoints = (p1, p2) => p1.row - p2.row || p1.column - p2.column
  ...
  Range.fromPoints = (start, end) => new Range(start.row, start.column, end.row, end.column)
  Range.comparePoints = comparePoints

  Range.comparePoints = (p1, p2) => p1.row - p2.row || p1.column - p2.column
...
ace.define(
  ...
  (require, exports, module) => {
    ...
    ;(function () {
      ...
      else this.$split = (text) => text.split(/\r\n|\r|\n/)
      ...
      this.insertInLine = function (position, text) {
        ...
        this.$lines[position.row] =
          line.substring(0, position.column) + text + line.substring(position.column)
    ...
    })(
      {
        ...
        2: [
          (_dereq_, module, exports) => {
            ...
            EntityParser.consumeEntity = function (buffer, tokenizer, additionalAllowedCharacter) {
              ...
              if (ch == "#") {
                ...
                if (isDigit(ch)) {
                  ...
                  code = Number.parseInt(code, radix)
            ...
            EntityParser.replaceEntityNumbers = (c) => {
              switch (c) {
                case 0x00:
            ...
            Tokenizer.prototype.tokenize = function (source) {
              ...
              function rcdata_end_tag_name_state(buffer) {
                var appropriate =
                  ...
                  tokenizer._currentToken.name === this._temporaryBuffer.toLowerCase()
                var data = buffer.char()
              ...
              function after_doctype_name_state(buffer) {
                ...
                if (data === InputStream.EOF) {
                  ...
                  if (["p", "P"].indexOf(data) > -1) {
                    ...
                    var matched = expected.every((expected) => {
                      data = buffer.char()
          ...
          (_dereq_, module, exports) => {
            ...
            function TreeBuilder() {
              ...
              modes.base = {
                ...
                processDoctype: (name, publicId, systemId, forceQuirks) => {
              ...
              modes.initial.processComment = (data) => {
                tree.insertComment(data, tree.document)
              }

              modes.initial.processDoctype = (name, publicId, systemId, forceQuirks) => {
                ...
                if (
                  ...
                  if (
                    ...
                    !((systemId == null || systemId == "about:legacy-compat") && publicId == null)
                  ) {
              ...
              modes.beforeHTML.processComment = (data) => {
                tree.insertComment(data, tree.document)
              }
              ...
              modes.beforeHTML.startTagHtml = (name, attributes, selfClosing) => {
              ...
              modes.afterAfterBody.processComment = (data) => {
                tree.insertComment(data, tree.document)
              }

              modes.afterAfterBody.processDoctype = (data) => {
                modes.inBody.processDoctype(data)
              }

              modes.afterAfterBody.startTagHtml = (data, attributes) => {
                modes.inBody.startTagHtml(data, attributes)
              }

              modes.afterAfterBody.startTagOther = (name, attributes, selfClosing) => {
                tree.parseError("unexpected-start-tag", { name: name })
                tree.setInsertionMode("inBody")
                tree.insertionMode.processStartTag(name, attributes, selfClosing)
              }

              modes.afterAfterBody.endTagOther = (name) => {
                tree.parseError("unexpected-end-tag", { name: name })
                tree.setInsertionMode("inBody")
                tree.insertionMode.processEndTag(name)
              }

              modes.afterAfterBody.processCharacters = (data) => {
              ...
              modes.afterBody.processComment = (data) => {
                tree.insertComment(data, tree.openElements.rootNode)
              }

              modes.afterBody.processCharacters = (data) => {
                if (!isAllWhitespace(data.characters)) {
                  tree.parseError("unexpected-char-after-body")
                  tree.setInsertionMode("inBody")
                  return tree.insertionMode.processCharacters(data)
              ...
              modes.afterBody.processStartTag = (name, attributes, selfClosing) => {
                tree.parseError("unexpected-start-tag-after-body", { name: name })
                tree.setInsertionMode("inBody")
                tree.insertionMode.processStartTag(name, attributes, selfClosing)
              }

              modes.afterBody.endTagHtml = (name) => {
                if (tree.context) {
              ...
              modes.afterBody.endTagOther = (name) => {
              ...
              modes.afterFrameset.processCharacters = (buffer) => {
              ...
              modes.afterFrameset.startTagNoframes = (name, attributes) => {
                modes.inHead.processStartTag(name, attributes)
              }

              modes.afterFrameset.startTagOther = (name, attributes) => {
                tree.parseError("unexpected-start-tag-after-frameset", { name: name })
              }

              modes.afterFrameset.endTagHtml = (name) => {
                tree.setInsertionMode("afterAfterFrameset")
              }

              modes.afterFrameset.endTagOther = (name) => {
              ...
              modes.beforeHead.startTagHead = (name, attributes) => {
              ...
              modes.beforeHead.endTagOther = (name) => {
              ...
              modes.inHead.startTagHtml = (name, attributes) => {
                modes.inBody.processStartTag(name, attributes)
              }

              modes.inHead.startTagHead = (name, attributes) => {
                tree.parseError("two-heads-are-not-better-than-one")
              }

              modes.inHead.startTagTitle = (name, attributes) => {
                tree.processGenericRCDATAStartTag(name, attributes)
              }

              modes.inHead.startTagNoScript = (name, attributes) => {
              ...
              modes.inHead.startTagNoFramesStyle = (name, attributes) => {
                tree.processGenericRawTextStartTag(name, attributes)
              }

              modes.inHead.startTagScript = (name, attributes) => {
                tree.insertElement(name, attributes)
              ...
              modes.inHead.startTagBaseBasefontBgsoundLink = (name, attributes) => {
                tree.insertSelfClosingElement(name, attributes)
              }

              modes.inHead.startTagMeta = (name, attributes) => {
                tree.insertSelfClosingElement(name, attributes)
              }
              ...
              modes.afterHead.startTagHtml = (name, attributes) => {
                modes.inBody.processStartTag(name, attributes)
              }

              modes.afterHead.startTagBody = (name, attributes) => {
                tree.framesetOk = false
                tree.insertBodyElement(attributes)
                tree.setInsertionMode("inBody")
              }

              modes.afterHead.startTagFrameset = (name, attributes) => {
                tree.insertElement(name, attributes)
                tree.setInsertionMode("inFrameset")
              }

              modes.afterHead.startTagFromHead = (name, attributes, selfClosing) => {
              ...
              modes.afterHead.startTagHead = (name, attributes, selfClosing) => {
                tree.parseError("unexpected-start-tag", { name: name })
              }
              ...
              modes.afterHead.endTagOther = (name) => {
                tree.parseError("unexpected-end-tag", { name: name })
              }

              modes.afterHead.anythingElse = () => {
              ...
              modes.inBody.processCharacters = (buffer) => {
                ...
                var characters = buffer.takeRemaining()
                characters = characters.replace(/\u0000/g, (match, index) => {
              ...
              modes.inBody.startTagProcessInHead = (name, attributes) => {
                modes.inHead.processStartTag(name, attributes)
              }

              modes.inBody.startTagBody = (name, attributes) => {
              ...
              modes.inBody.startTagFrameset = (name, attributes) => {
              ...
              modes.inBody.startTagRpRt = (name, attributes) => {
              ...
              modes.inBody.startTagA = (name, attributes) => {
              ...
              modes.inBody.startTagFormatting = (name, attributes) => {
                tree.reconstructActiveFormattingElements()
                tree.insertFormattingElement(name, attributes)
              }
              ...
              modes.inBody.startTagAppletMarqueeObject = (name, attributes) => {
                tree.reconstructActiveFormattingElements()
                tree.insertElement(name, attributes)
              ...
              modes.inBody.endTagAppletMarqueeObject = (name) => {
              ...
              modes.inBody.startTagVoidFormatting = (name, attributes) => {
                tree.reconstructActiveFormattingElements()
                tree.insertSelfClosingElement(name, attributes)
                tree.framesetOk = false
              }

              modes.inBody.startTagParamSourceTrack = (name, attributes) => {
                tree.insertSelfClosingElement(name, attributes)
              }
              ...
              modes.inBody.startTagTextarea = (name, attributes) => {
                tree.insertElement(name, attributes)
              ...
              modes.inBody.startTagRawText = (name, attributes) => {
                tree.processGenericRawTextStartTag(name, attributes)
              }

              modes.inBody.startTagSelect = (name, attributes) => {
                tree.reconstructActiveFormattingElements()
                tree.insertElement(name, attributes)
                tree.framesetOk = false
              ...
              modes.inBody.startTagMisplaced = (name, attributes) => {
                tree.parseError("unexpected-start-tag-ignored", { name: name })
              }

              modes.inBody.endTagMisplaced = (name) => {
                tree.parseError("unexpected-end-tag", { name: name })
              }

              modes.inBody.endTagBr = (name) => {
              ...
              modes.inBody.startTagOptionOptgroup = (name, attributes) => {
                if (tree.currentStackItem().localName == "option") tree.popElement()
                tree.reconstructActiveFormattingElements()
                tree.insertElement(name, attributes)
              }

              modes.inBody.startTagOther = (name, attributes) => {
                tree.reconstructActiveFormattingElements()
                tree.insertElement(name, attributes)
              }

              modes.inBody.endTagOther = (name) => {
                var node
                for (var i = tree.openElements.length - 1; i > 0; i--) {
                  ...
                  if (node.localName == name) {
                    ...
                    if (tree.currentStackItem().localName != name)
                      tree.parseError("unexpected-end-tag", { name: name })
                    tree.openElements.remove_openElements_until((x) => x === node)
                    break
                  }
                  if (node.isSpecial()) {
                    tree.parseError("unexpected-end-tag", { name: name })
              ...
              modes.inBody.startTagMath = (name, attributes, selfClosing) => {
                tree.reconstructActiveFormattingElements()
              ...
              modes.inBody.startTagSVG = (name, attributes, selfClosing) => {
                tree.reconstructActiveFormattingElements()
              ...
              modes.inBody.endTagListItem = (name) => {
              ...
              modes.inBody.endTagHeading = (name) => {
                ...
                tree.openElements.remove_openElements_until((e) => e.isNumberedHeader())
              }
              ...
              modes.inCaption.processCharacters = (data) => {
                modes.inBody.processCharacters(data)
              }

              modes.inCaption.startTagTableElement = (name, attributes) => {
                tree.parseError("unexpected-end-tag", { name: name })
              ...
              modes.inCaption.startTagOther = (name, attributes, selfClosing) => {
                modes.inBody.processStartTag(name, attributes, selfClosing)
              }

              modes.inCaption.endTagCaption = (name) => {
              ...
              modes.inCaption.endTagTable = (name) => {
              ...
              modes.inCaption.endTagIgnore = (name) => {
                tree.parseError("unexpected-end-tag", { name: name })
              }

              modes.inCaption.endTagOther = (name) => {
              ...
              modes.inCell.processCharacters = (data) => {
                modes.inBody.processCharacters(data)
              }
              ...
              modes.inCell.startTagOther = (name, attributes, selfClosing) => {
                modes.inBody.processStartTag(name, attributes, selfClosing)
              }

              modes.inCell.endTagTableCell = (name) => {
              ...
              modes.inCell.endTagIgnore = (name) => {
                tree.parseError("unexpected-end-tag", { name: name })
              }
              ...
              modes.inColumnGroup.ignoreEndTagColgroup = () =>
                tree.currentStackItem().localName == "html"
              ...
              modes.inColumnGroup.startTagCol = (name, attributes) => {
                tree.insertSelfClosingElement(name, attributes)
              }
              ...
              modes.inColumnGroup.endTagCol = (name) => {
              ...
              modes.inForeignContent.processStartTag = (name, attributes, selfClosing) => {
                if (
                  [
                    "b",
                    "big",
                  ...
                  (name == "font" &&
                    attributes.some(
                      (attr) => ["color", "face", "size"].indexOf(attr.nodeName) >= 0,
                    ))
                ) {
              ...
              modes.inForeignContent.processEndTag = (name) => {
              ...
              modes.inForeignContent.processCharacters = (buffer) => {
                var characters = buffer.takeRemaining()
                characters = characters.replace(/\u0000/g, (match, index) => {
                  tree.parseError("invalid-codepoint")
                  return "\uFFFD"
              ...
              modes.inHeadNoscript.processComment = (data) => {
              ...
              modes.inHeadNoscript.startTagHeadNoscript = (name, attributes) => {
              ...
              modes.inHeadNoscript.endTagNoscript = (name, attributes) => {
                tree.popElement()
                tree.setInsertionMode("inHead")
              }

              modes.inHeadNoscript.endTagOther = (name, attributes) => {
                tree.parseError("unexpected-end-tag-in-frameset", { name: name })
              }

              modes.inHeadNoscript.anythingElse = () => {
                tree.popElement()
                tree.setInsertionMode("inHead")
              }
              ...
              modes.inFrameset.processCharacters = (data) => {
                tree.parseError("unexpected-char-in-frameset")
              }

              modes.inFrameset.startTagFrameset = (name, attributes) => {
                tree.insertElement(name, attributes)
              }

              modes.inFrameset.startTagFrame = (name, attributes) => {
                tree.insertSelfClosingElement(name, attributes)
              }

              modes.inFrameset.startTagNoframes = (name, attributes) => {
                modes.inBody.processStartTag(name, attributes)
              }

              modes.inFrameset.startTagOther = (name, attributes) => {
                tree.parseError("unexpected-start-tag-in-frameset", { name: name })
              }

              modes.inFrameset.endTagFrameset = (name, attributes) => {
              ...
              modes.inFrameset.endTagNoframes = (name) => {
                modes.inBody.processEndTag(name)
              }

              modes.inFrameset.endTagOther = (name) => {
                tree.parseError("unexpected-end-tag-in-frameset", { name: name })
              }
              ...
              modes.inTable.startTagCaption = (name, attributes) => {
              ...
              modes.inTable.startTagColgroup = (name, attributes) => {
                tree.openElements.popUntilTableScopeMarker()
                tree.insertElement(name, attributes)
              ...
              modes.inTable.startTagRowGroup = (name, attributes) => {
                tree.openElements.popUntilTableScopeMarker()
                tree.insertElement(name, attributes)
              ...
              modes.inTable.startTagTable = (name, attributes) => {
                tree.parseError("unexpected-start-tag-implies-end-tag", {
                  startName: "table",
                  endName: "table",
                })
              ...
              modes.inTable.startTagStyleScript = (name, attributes) => {
                modes.inHead.processStartTag(name, attributes)
              }
              ...
              modes.inTable.startTagForm = (name, attributes) => {
              ...
              modes.inTable.startTagOther = (name, attributes, selfClosing) => {
              ...
              modes.inTable.endTagTable = (name) => {
                if (tree.openElements.inTableScope(name)) {
                  tree.generateImpliedEndTags()
                  if (tree.currentStackItem().localName != name) {
              ...
              modes.inTable.endTagIgnore = (name) => {
                tree.parseError("unexpected-end-tag", { name: name })
              }

              modes.inTable.endTagOther = (name) => {
              ...
              modes.inTableText.flushCharacters = () => {
              ...
              modes.inTableText.processCharacters = (buffer) => {
                var characters = buffer.takeRemaining()
                characters = characters.replace(/\u0000/g, (match, index) => {
                  tree.parseError("invalid-codepoint")
                  return ""
              ...
              modes.inTableBody.processCharacters = (data) => {
                modes.inTable.processCharacters(data)
              }

              modes.inTableBody.startTagTr = (name, attributes) => {
              ...
              modes.inTableBody.startTagOther = (name, attributes) => {
                modes.inTable.processStartTag(name, attributes)
              }

              modes.inTableBody.endTagTableRowGroup = (name) => {
                if (tree.openElements.inTableScope(name)) {
                  tree.openElements.popUntilTableBodyScopeMarker()
                  tree.popElement()
                  tree.setInsertionMode("inTable")
              ...
              modes.inTableBody.endTagIgnore = (name) => {
                tree.parseError("unexpected-end-tag-in-table-body", { name: name })
              }

              modes.inTableBody.endTagOther = (name) => {
              ...
              modes.inSelect.processCharacters = (buffer) => {
                var data = buffer.takeRemaining()
                data = data.replace(/\u0000/g, (match, index) => {
                  tree.parseError("invalid-codepoint")
                  return ""
                })
                if (!data) return
              ...
              modes.inSelect.startTagOption = (name, attributes) => {
                if (tree.currentStackItem().localName == "option") tree.popElement()
                tree.insertElement(name, attributes)
              }

              modes.inSelect.startTagOptgroup = (name, attributes) => {
                if (tree.currentStackItem().localName == "option") tree.popElement()
                if (tree.currentStackItem().localName == "optgroup") tree.popElement()
                tree.insertElement(name, attributes)
              }

              modes.inSelect.endTagOption = (name) => {
              ...
              modes.inSelect.endTagOptgroup = (name) => {
                if (
              ...
              modes.inSelect.endTagSelect = (name) => {
              ...
              modes.inSelect.startTagScript = (name, attributes) => {
                modes.inHead.processStartTag(name, attributes)
              }
              ...
              modes.inSelect.startTagOther = (name, attributes) => {
                tree.parseError("unexpected-start-tag-in-select", { name: name })
              }

              modes.inSelect.endTagOther = (name) => {
                tree.parseError("unexpected-end-tag-in-select", { name: name })
              }
              ...
              modes.inSelectInTable.processCharacters = (data) => {
              ...
              modes.inSelectInTable.startTagOther = (name, attributes, selfClosing) => {
              ...
              modes.inSelectInTable.endTagOther = (name) => {
              ...
              modes.inRow.processCharacters = (data) => {
                modes.inTable.processCharacters(data)
              }

              modes.inRow.startTagTableCell = (name, attributes) => {
              ...
              modes.inRow.startTagOther = (name, attributes, selfClosing) => {
              ...
              modes.inRow.endTagIgnore = (name) => {
                tree.parseError("unexpected-end-tag-in-table-row", { name: name })
              }

              modes.inRow.endTagOther = (name) => {
                modes.inTable.processEndTag(name)
              }

              modes.inRow.ignoreEndTagTr = () => !tree.openElements.inTableScope("tr")
              ...
              modes.afterAfterFrameset.processEOF = () => {}

              modes.afterAfterFrameset.processComment = (data) => {
                tree.insertComment(data, tree.document)
              }

              modes.afterAfterFrameset.processCharacters = (buffer) => {
                var characters = buffer.takeRemaining()
                var whitespace = ""
                for (var i = 0; i < characters.length; i++) {
                  var ch = characters[i]
              ...
              modes.afterAfterFrameset.startTagNoFrames = (name, attributes) => {
                modes.inHead.processStartTag(name, attributes)
              }

              modes.afterAfterFrameset.startTagOther = (name, attributes, selfClosing) => {
                tree.parseError("expected-eof-but-got-start-tag", { name: name })
              }

              modes.afterAfterFrameset.processEndTag = (name, attributes) => {
              ...
              modes.text.processCharacters = (buffer) => {
                if (tree.shouldSkipLeadingNewline) {
                  tree.shouldSkipLeadingNewline = false
                  buffer.skipAtMostOneLeadingNewline()
                }
            ...
            TreeBuilder.prototype.adoptionAgencyEndTag = function (name) {
              ...
              while (outerLoopCounter++ < outerIterationLimit) {
                ...
                var furthestBlock = this.openElements.furthestBlockForFormattingElement(
                ...
                while (innerLoopCounter++ < innerIterationLimit) {
                  ...
                  this.activeFormattingElements[this.activeFormattingElements.indexOf(node)] =
                    newNode
            ...
            TreeBuilder.prototype.createElement = (namespaceURI, localName, attributes) => {
              throw new Error("Not implemented")
            }

            TreeBuilder.prototype.attachNode = (child, parent) => {
              throw new Error("Not implemented")
            }

            TreeBuilder.prototype.attachNodeToFosterParent = (child, table, stackParent) => {
              throw new Error("Not implemented")
            }

            TreeBuilder.prototype.detachFromParent = (node) => {
              throw new Error("Not implemented")
            }

            TreeBuilder.prototype.addAttributesToElement = (element, attributes) => {
              throw new Error("Not implemented")
            }
            ...
            TreeBuilder.prototype.insertHtmlElement = function (attributes) {
              ...
              this.openElements.pushHtmlElement(
                new StackItem("http://www.w3.org/1999/xhtml", "html", attributes, root),
              )
              return root
            }
            ...
            TreeBuilder.prototype.insertComment = (data, parent) => {
              throw new Error("Not implemented")
            }

            TreeBuilder.prototype.insertDoctype = (name, publicId, systemId) => {
              throw new Error("Not implemented")
            }

            TreeBuilder.prototype.insertText = (data) => {
              throw new Error("Not implemented")
            }
            ...
            TreeBuilder.prototype.generateImpliedEndTags = function (exclude) {
              ...
              if (
                ["dd", "dt", "li", "option", "optgroup", "p", "rp", "rt"].indexOf(name) != -1 &&
                name != exclude
              ) {
                this.popElement()
            ...
            TreeBuilder.prototype.reconstructActiveFormattingElements = function () {
              ...
              while (true) {
                ...
                if (
                  element == this.activeFormattingElements[this.activeFormattingElements.length - 1]
                )
                  break
              }
            }
            ...
            TreeBuilder.prototype.reparentChildren = (oldParent, newParent) => {
              throw new Error("Not implemented")
            }
            ...
            TreeBuilder.prototype.adjustMathMLAttributes = (attributes) => {
            ...
            TreeBuilder.prototype.adjustSVGTagNameCase = (name) => constants.SVGTagMap[name] || name

            TreeBuilder.prototype.adjustSVGAttributes = (attributes) => {
              attributes.forEach((a) => {
            ...
            TreeBuilder.prototype.adjustForeignAttributes = (attributes) => {
              for (var i = 0; i < attributes.length; i++) {
                var attribute = attributes[i]
            ...
            function formatMessage(format, args) {
              return format.replace(/{[0-9a-z-]+}/gi, (match) => args[match.slice(1, -1)] || match)
            }
        ...
        10: [
          (_dereq_, module, exports) => {
            ...
            SAXTreeBuilder.prototype.attachNode = (node, parent) => {
              parent.appendChild(node)
            }

            SAXTreeBuilder.prototype.attachNodeToFosterParent = (child, table, stackParent) => {
              var parent = table.parentNode
              if (parent) parent.insertBetween(child, table.previousSibling, table)
              else stackParent.appendChild(child)
            }

            SAXTreeBuilder.prototype.detachFromParent = (element) => {
              element.detach()
            }

            SAXTreeBuilder.prototype.reparentChildren = (oldParent, newParent) => {
            ...
            SAXTreeBuilder.prototype.addAttributesToElement = (element, attributes) => {
              for (var i = 0; i < attributes.length; i++) {
                var attribute = attributes[i]
            ...
            Node.prototype.visit = (treeParser) => {
              throw new Error("Not Implemented")
            }
            Node.prototype.revisit = (treeParser) => {
              return
            }
            ...
            DocumentFragment.prototype.visit = (treeParser) => {}
            ...
            ProcessingInstruction.prototype.getNodeType = () => NodeType.PROCESSING_INSTRUCTION
            ...
            DTD.prototype.revisit = (treeParser) => {
              treeParser.endDTD()
            }
        ...
        11: [
          (_dereq_, module, exports) => {
            ...
            NullLexicalHandler.prototype.comment = () => {}
            NullLexicalHandler.prototype.endCDATA = () => {}
            NullLexicalHandler.prototype.endDTD = () => {}
            NullLexicalHandler.prototype.endEntity = () => {}
            NullLexicalHandler.prototype.startCDATA = () => {}
            NullLexicalHandler.prototype.startDTD = () => {}
            NullLexicalHandler.prototype.startEntity = () => {}
  ...
  (require, exports, module) => {
    ...
    ;(function () {
      ...
      this.onUpdate = function () {
        ...
        if (this.context) parser.parseFragment(value, this.context)
    ...
    })(
      {
        ...
        2: [
          function (_dereq_, module, exports) {
            ;(function () {
              ...
              _.iteratee = (value, context) => cb(value, context, Number.POSITIVE_INFINITY)
              var createAssigner = (keysFunc, undefinedOnly) => (obj) => {
              ...
              _.find = _.detect = (obj, predicate, context) => {
                var key
                if (isArrayLike(obj)) {
              ...
              _.filter = _.select = (obj, predicate, context) => {
                var results = []
              ...
              _.every = _.all = (obj, predicate, context) => {
                predicate = cb(predicate, context)
                var keys = !isArrayLike(obj) && _.keys(obj),
                  length = (keys || obj).length
                for (var index = 0; index < length; index++) {
              ...
              _.some = _.any = (obj, predicate, context) => {
                predicate = cb(predicate, context)
                var keys = !isArrayLike(obj) && _.keys(obj),
                  length = (keys || obj).length
                for (var index = 0; index < length; index++) {
              ...
              _.pluck = (obj, key) => _.map(obj, _.property(key))
              _.where = (obj, attrs) => _.filter(obj, _.matcher(attrs))
              _.findWhere = (obj, attrs) => _.find(obj, _.matcher(attrs))
              _.max = (obj, iteratee, context) => {
                var result = Number.NEGATIVE_INFINITY,
                  lastComputed = Number.NEGATIVE_INFINITY,
                  value,
                  computed
                if (iteratee == null && obj != null) {
                  ...
                  _.each(obj, (value, index, list) => {
                    ...
                    if (
                      ...
                      (computed === Number.NEGATIVE_INFINITY && result === Number.NEGATIVE_INFINITY)
                    ) {
                      result = value
              ...
              _.min = (obj, iteratee, context) => {
                var result = Number.POSITIVE_INFINITY,
                  lastComputed = Number.POSITIVE_INFINITY,
                  value,
                  computed
                if (iteratee == null && obj != null) {
                  obj = isArrayLike(obj) ? obj : _.values(obj)
                  ...
                  _.each(obj, (value, index, list) => {
                    ...
                    if (
                      computed < lastComputed ||
                      (computed === Number.POSITIVE_INFINITY && result === Number.POSITIVE_INFINITY)
                    ) {
                      result = value
                      lastComputed = computed
                    }
              ...
              var group = (behavior) => (obj, iteratee, context) => {
                var result = {}
                iteratee = cb(iteratee, context)
              ...
              _.initial = (array, n, guard) =>
                slice.call(array, 0, Math.max(0, array.length - (n == null || guard ? 1 : n)))
              ...
              var flatten = (input, shallow, strict, startIndex) => {
                var output = [],
                  idx = 0
              ...
              var executeBound = (sourceFunc, boundFunc, context, callingContext, args) => {
              ...
              _.wrap = (func, wrapper) => _.partial(wrapper, func)
              _.negate = (predicate) =>
                function () {
              ...
              _.isFinite = (obj) => isFinite(obj) && !isNaN(Number.parseFloat(obj))
              ...
              _.property = (key) => (obj) => (obj == null ? void 0 : obj[key])
              _.propertyOf = (obj) => (obj == null ? () => {} : (key) => obj[key])
              _.matcher = _.matches = (attrs) => {
              ...
              var result = (instance, obj) => (instance._chain ? _(obj).chain() : obj)
              _.mixin = (obj) => {
          ...
          (_dereq_, module, exports) => {
            ...
            var JSHINT = (() => {
              ...
              function assume() {
                ...
                state.option.inMoz = (strict) => state.option.moz

                state.option.inESNext = (strict) => state.option.moz || state.option.esnext

                state.option.inES5 = (/* strict */) => !state.option.es3

                state.option.inES3 = (strict) => {
                  if (strict) {
              ...
              function removeIgnoredMessages() {
                ...
                JSHINT.errors = _.reject(JSHINT.errors, (err) => ignored[err.line])
              }
              ...
              function isEndOfExpr() {
                ...
                if (
                  ...
                  (curr.id === "yield" && state.option.inMoz(true))
                ) {
                        ...
                        if (left) {
                          ...
                          if (left.id === ".") {
                            ...
                            if (state.tokens.curr.left.first) {
                              state.tokens.curr.left.first.forEach((t) => {
              ...
              reservevar("this", (x) => {
                if (
                  ...
                  ((funct["(statement)"] && funct["(name)"].charAt(0) > "Z") || funct["(global)"])
                ) {
                  warning("W040", x)
                }
              })
              ...
              relation("==", function (left, right) {
                var eqnull =
                  state.option.eqnull && (left.value === "null" || right.value === "null")
              ...
              infix(
                ...
                (left, that) => {
                  ...
                  if (left && left.value === "arguments" && (m === "callee" || m === "caller")) {
                    ...
                    else if (state.directive["use strict"]) error("E008")
                  } else if (
                    !state.option.evil &&
                    left &&
                    left.value === "document" &&
                  ...
                  if (left) {
                    if (left.type === "(identifier)") {
                      if (left.value.match(/^[A-Z]([A-Z0-9_$]*[a-z][A-Za-z0-9_$]*)?$/)) {
                        if (
                          "Number String Boolean Date Object Error Symbol".indexOf(left.value) ===
                          -1
                        ) {
                          if (left.value === "Math") {
                            warning("W063", left)
              ...
              function createMetrics(functionStartToken) {
                return {
                  ...
                  verifyMaxParametersPerFunction: (params) => {
                    params = params || []
              ...
              function destructuringExpressionMatch(tokens, value) {
                ...
                _.zip(tokens, Array.isArray(first) ? first : [first]).forEach((val) => {
                  var token = val[0]
                  var value = val[1]
                    ...
                    if (tokens.hasOwnProperty(t)) {
                      ...
                      if (funct["(global)"]) {
                        if (predefined[t.id] === false) {
                          ...
                          if (
                            (!state.option.inES5() && vars.ecmaIdentifiers[5][t.id] === false) ||
              ...
              prefix("function", function () {
                ...
                if (!state.option.loopfunc && funct["(loopage)"]) {
                  if (_.some(fn, (val, name) => isVariable(name) && !isLocal(name))) {
                    warning("W083")
                  }
                }
                return this
                  ...
                  if (state.tokens.next.id === "var") {
                    ...
                    switch (funct[state.tokens.next.value]) {
                      ...
                      default:
                        ...
                        if (
                          !funct["(blockscope)"].getlabel(ident) &&
                          !(scope[ident] || {})[ident]
                        ) {
              ...
              var lookupBlockType = () => {
                ...
                var bracketStack = 0
                var ret = {}
                if (checkPunctuators(state.tokens.curr, ["[", "{"])) bracketStack += 1
                do {
              ...
              var arrayComprehension = () => {
                ...
                var _current
                function declare(v) {
                  var l = _current.variables.filter((elt) => {
                    if (elt.value === v) {
                      elt.undef = false
                      return v
                    }
              ...
              var itself = (s, o, g) => {
                ...
                api = {
                  ...
                  getOption: (name) => state.option[name] || null,

                  getCache: (name) => state.cache[name],
                ...
                if (o && o.ignoreDelimiters) {
                  ...
                  o.ignoreDelimiters.forEach((delimiterPair) => {
                    ...
                    s = s.replace(reIgnore, (match) => match.replace(/./g, " "))
                  })
                }
                ...
                try {
                    ...
                    default:
                      ...
                      if (state.directive["use strict"]) {
                        if (!state.option.globalstrict) {
                          if (
                            !(state.option.node || state.option.phantom || state.option.browserify)
                          ) {
                  ...
                  advance(
                    state.tokens.next && state.tokens.next.value !== "." ? "(end)" : undefined,
                  )
                  funct["(blockscope)"].unstack()

                  var markDefined = (name, context) => {
                    do {
                      if (typeof context[name] === "string") {
                        if (context[name] === "unused") context[name] = "var"
                        else if (context[name] === "unction") context[name] = "closure"
                  ...
                  var clearImplied = (name, line) => {
                    ...
                    if (newImplied.length === 0) delete implied[name]
                    else implied[name] = newImplied
                  }

                  var warnUnused = (name, tkn, type, unused_opt) => {
                    var line = tkn.line
                    var chr = tkn.from
            ...
            Lexer.prototype = {
              ...
              scanIdentifier: function () {
                ...
                var readUnicodeEscapeSequence = function () {
                  ...
                  if (isHexDigit(ch1) && isHexDigit(ch2) && isHexDigit(ch3) && isHexDigit(ch4)) {
                    code = Number.parseInt(ch1 + ch2 + ch3 + ch4, 16)
                ...
                function removeEscapeSequences(id) {
                  return id.replace(/\\u([0-9a-fA-F]{4})/g, (m0, codepoint) =>
              ...
              scanEscapeSequence: function (checks) {
                ...
                switch (char) {
                  ...
                  case "0":
                    char = "\\0"
                    var n = Number.parseInt(this.peek(1), 10)
                    this.triggerAsync(
                      "warning",
                      {
                        code: "W115",
                      ...
                      checks,
                      () => n >= 0 && n <= 7 && state.directive["use strict"],
                    )
                    break
                  case "u":
                    var hexCode = this.input.substr(1, 4)
                    var code = Number.parseInt(hexCode, 16)
                    if (isNaN(code)) {
                      this.trigger("warning", {
                        code: "W052",
                        line: this.line,
                  ...
                  case "v":
                    ...
                    break
                  case "x":
                    var x = Number.parseInt(this.input.substr(1, 2), 16)
              ...
              nextLine: function () {
                ...
                var startsWith = () =>
                  _.some(arguments, (prefix) => inputTrimmed.indexOf(prefix) === 0)

                var endsWith = () =>
                  _.some(
                    arguments,
                    (suffix) =>
                      inputTrimmed.indexOf(suffix, inputTrimmed.length - suffix.length) !== -1,
                  )
                  ...
                  switch (token.type) {
                    ...
                    case Token.NumericLiteral:
                      ...
                      this.triggerAsync(
                        ...
                        checks,
                        () => state.directive["use strict"] && token.base === 8 && token.isLegacy,
                      )
        ...
        5: [
          (_dereq_, module, exports) => {
            ...
            _.each(errors, (desc, code) => {
              exports.errors[code] = { code: code, desc: desc }
            })

            _.each(warnings, (desc, code) => {
              exports.warnings[code] = { code: code, desc: desc }
            })

            _.each(info, (desc, code) => {
  ...
  (require, exports, module) => {
    ...
    ;(function () {
      ...
      this.onUpdate = function () {
        ...
        value = value.replace(/^#!.*\n/, "\n")
        if (!value) return this.sender.emit("error", []) //changed
...
function GetJsFromMixedHtml(s, debug) {
  var r = ""
  ;(d = ""), (inScript = false), (lines = s.split("\n"))
