  ...
  (require, exports, module) => {
    ...
    ;(function () {
      ...
      this.getTokenizer = () => {
        function TabstopToken(str, _, stack) {
          ...
          if (/^\d+$/.test(str) && !stack.inFormatString)
            return [{ tabstopId: Number.parseInt(str, 10) }]
          return [{ text: str }]
        }
        function escape(ch) {
          return "(?:[^\\\\" + ch + "]|\\\\.)"
        ...
        SnippetManager.$tokenizer = new Tokenizer({
          start: [
            ...
            { regex: /}/, onMatch: (val, state, stack) => [stack.length ? stack.shift() : val] },
      ...
      this.tmStrFormat = function (str, ch, editor) {
        ...
        var formatted = str.replace(re, () => {
          this.variables.__ = arguments
          var fmtParts = this.resolveVariables(fmtTokens, editor)
          var gChangeCase = "E"
      ...
      this.insertSnippetForSelection = function (editor, snippetText) {
        ...
        tokens.forEach((p, i) => {
          ...
          var isNested = value.some((t) => typeof t === "object")
      ...
      this.expandWithTab = function (editor, options) {
        var result = editor.forEachSelection(
          () => this.expandSnippetForSelection(editor, options),
          null,
          { keepOrder: true },
        )
    ...
    ;(function () {
      ...
      this.onChange = function (delta) {
        ...
        if (!this.$inChange && isRemove) {
          ...
          var changedOutside =
            ts &&
            !ts.some((r) => comparePoints(r.start, start) <= 0 && comparePoints(r.end, end) >= 0)
...
ace.define(
  ...
  (require, exports, module) => {
    ...
    var AcePopup = (parentNode) => {
      ...
      var lastMouseEvent
      var hoverMarker = new Range(-1, 0, -1, Number.POSITIVE_INFINITY)
      var selectionMarker = new Range(-1, 0, -1, Number.POSITIVE_INFINITY)
      // there we can add our class for active line in aoutocomplete modal
      selectionMarker.id = popup.session.addMarker(
        selectionMarker,
        "ace_active-line oo_highlight",
        "fullLine",
      )
      ...
      popup.$blockScrolling = Number.POSITIVE_INFINITY
      popup.isOpen = false
  ...
  (require, exports, module) => {
    ...
    ;(function () {
      ...
      this.commands = {
        ...
        "Ctrl-Up|Ctrl-Home": (editor) => {
          editor.completer.goTo("start")
        },
        "Ctrl-Down|Ctrl-End": (editor) => {
  ...
  (require, exports, module) => {
    var TernServer = function (options) {
      ...
      if (!this.options.hasOwnProperty("switchToDoc"))
        this.options.switchToDoc = (name, start) => {
          console.log(
      ...
      if (this.options.useWorker) {
        ...
        this.server = new tern.Server({
          getFile: (name, c) => getFile(this, name, c),
          async: true,
      ...
      this.trackChange = (change, doc) => {
        trackChange(this, doc, change)
      }
      ...
      if (this.options.queryTimeout && !isNaN(Number.parseInt(this.options.queryTimeout)))
        this.queryTimeout = Number.parseInt(this.options.queryTimeout)
      ...
      ternRefresh: {
        ...
        exec: (editor) => {
          ...
          if (editor.ternServer.refreshDocLastCalled != null) {
            if (new Date().getTime() - editor.ternServer.refreshDocLastCalled < 1000) {
              //less than 1 second
              full = true
            }
          }
    ...
    TernServer.prototype = {
      ...
      refreshDoc: function (editor, full) {
        ...
        sendDoc(this, doc)
        showTip(
          'Tern document refreshed <div style="color:gray;">(press hotkey twice in  &lt; 1 second to do a full reload including refs)</div>',
        )
      },
      ...
      enabledAtCurrentLocation: (editor) => inJavascriptMode(editor),
      getCallPos: (editor, pos) => getCallPos(editor, pos),
      docChanged: function (editor) {
        ...
        var finish = (name) => {
          this.addDoc(name, editor) //add current doc
    ...
    function getFragmentAround(data, start, end) {
      ...
      if (
        minIndent == null ||
        minIndent == countColumn(editor.session.getLine(start.line), null, tabSize)
      )
        endLine = max
      else
    ...
    function getCompletions(ts, editor, session, pos, prefix, callback) {
      var autoCompleteFiredTwiceInThreshold = () => {
        try {
      ...
      ts.request(
        ...
        (error, data) => {
          ...
          var ternCompletions = data.completions.map((item) => ({
          ...
          if (
            ...
            try {
              ts.aceTextCompletor.getCompletions(editor, session, pos, prefix, (error, data) => {
                ...
                var otherCompletionsContains = (value, minLength) => {
          ...
          function popupSelectionChanged() {
            ...
            if (!editor.completer.popup) return
        ...
        if (ts.options.typeTip) {
          ...
          if (data.hasOwnProperty("type")) {
            ...
            if (data.type.toString().length > 1 && data.type.toString().substr(0, 2) !== "fn") {
              var innerCB = (error, definitionData) => {
    ...
    function createInfoDataTip(data, includeType, activeArg) {
      ...
      if (includeType) {
        ...
        if (fnArgs) {
          ...
          var getParamDetailedName = (param) => {
            var name = param.name
          ...
          var useDetailedArgHints = params.length === 0 || !isNaN(Number.parseInt(activeArg))
          var typeStr = ""
          ...
          for (var i = 0; i < fnArgs.args.length; i++) {
            var paramStr = ""
            var isCurrent = !isNaN(Number.parseInt(activeArg)) ? i === activeArg : false
            ...
            if (!useDetailedArgHints) {
              ...
              paramStr += type
                ? '<span class="' + cls + 'type">' + htmlEncode(type) + "</span> "
                : ""
              paramStr +=
                '<span class="' +
                cls +
          ...
          if (fnArgs.rettype) {
            if (useDetailedArgHints) {
              typeStr +=
                ' -> <span class="' + cls + 'type">' + htmlEncode(fnArgs.rettype) + "</span>"
            } else {
            ...
            if (activeParamChildren && activeParamChildren.length > 0) {
              for (var i = 0; i < activeParamChildren.length; i++) {
                var t = activeParamChildren[i].type
                  ? '<span class="' + cls + 'type">{' + activeParamChildren[i].type + "} </span>"
                  : ""
                typeStr +=
                  '<div class="' +
                  cls +
      ...
      if (isNaN(Number.parseInt(activeArg))) {
        if (data.doc) {
          var replaceParams = (str, params) => {
            ...
            for (var i = 0; i < params.length; i++) {
              ...
              paramStr +=
                params[i].type.trim() === ""
                  ? ""
                  : '<span class="' + cls + 'type">{' + params[i].type + "}</span> "
          ...
          var highlighTags = (str) => {
            try {
              ...
              while ((m = re.exec(str)) !== null) {
                ...
                str = str.replace(
                  m[0],
                  ' <span class="' + cls + 'jsdoc-tag">' + m[0].trim() + "</span> ",
                )
              }
            } catch (ex) {
          ...
          var createLinks = (str) => {
            try {
              ...
              while ((m = re.exec(str)) !== null) {
                ...
                var text = m[0].replace(/https:\/\//i, "").replace(/http:\/\//i, "")
                str = str.replace(
                  m[0],
                  '<a class="' +
                    cls +
            ...
            (error, data) => {
              ...
              applyChanges(ts, data.changes, (result) => {
                var resultTip = makeTooltip(
                  null,
                  null,
                  elt("div", "", "Replaced " + result.replaced + " references sucessfully"),
                  editor,
                  true,
                )
                var errors = elt("div", "")
        ...
        for (var i = 0; i < chs.length; ++i) {
          try {
            ...
            known.doc.session.replace(
              new Range(ch.start.row, ch.start.column, ch.end.row, ch.end.column),
              ch.text,
            )
            result.replaced++
          } catch (ex) {
      ...
      function inner() {
        ts.request(
          ...
          (error, data) => {
            if (error) {
              if (
                error.toString().toLowerCase().indexOf("no expression at") === -1 &&
                error.toString().toLowerCase().indexOf("no type found at") === -1
              ) {
                return showError(ts, editor, error)
              }
            }
    ...
    function makeTooltip(x, y, content, editor, closeOnCusorActivity, fadeOutDuration) {
      ...
      if (editor.$tooltipContainer) {
        container = document.querySelector(editor.$tooltipContainer)
      } else {
      ...
      if (fadeOutDuration) {
        fadeOutDuration = Number.parseInt(fadeOutDuration, 10)
    ...
    function showError(ts, editor, msg, noPopup) {
      try {
        ...
        var isError = (o) => o && o.name && o.stack && o.message
    ...
    function findContext(editor, data) {
      ...
      return data
      console.log(
        new Error("This part is not complete, need to implement using Ace's search functionality"),
      )
    ...
    function loadExplicitVsRefs(ts, editor) {
      ...
      var isBrowser =
        window && window.location && window.location.toString().toLowerCase().indexOf("http") === 0
              ...
              if (xhr.readyState == 4) {
                if (xhr.status == 200) {
                  ...
                  if (xhr.status == 404) {
                    ...
                    addFileDone(
                      "error adding web reference (unknown error, see console): " + path,
                      true,
                    )
                  }
                }
            ...
            resolveFilePath(ts, path, (resolvedPath) => {
              getFile(ts, resolvedPath, (err, data) => {
                if (err || !data) {
                  ...
                  addFileDone(
                    "error getting file: " + resolvedPath + "(see console for details)",
                    true,
                  )
                } else {
...
ace.define(
  ...
  (require, exports, module) => {
    ...
    var onCursorChange_Tern = (e, editor_getSession_selection) => {
    ...
    var onAfterExec_Tern = (e, commandManager) => {
