...
use wo_ooxml::model::{DocxBlock, DocxParagraph, DocxParagraphProperties, DocxRun, DocxTableCell, DocxTableRow};
...
impl<'a> DocModel<'a> {
    ...
    pub fn table_apply_insert_row(&mut self, table_idx: usize, after_row: usize) -> Result<DocOp, DocOpError> {
        // Validate table index
        let table_block = self.body.get_block_mut(table_idx)
        ...
        let new_row = DocxTableRow {
            cells: template_row.cells.iter().map(|cell| DocxTableCell {
        ...
        Ok(DocOp::DeleteTableRow { table: table_idx, row: insert_pos })
    }
    
    /// Apply DeleteTableRow operation
    pub fn table_apply_delete_row(&mut self, table_idx: usize, row_idx: usize) -> Result<DocOp, DocOpError> {
        // Validate table index
        let table_block = self.body.get_block_mut(table_idx)
            .ok_or_else(|| DocOpError::TableIndexOutOfRange(table_idx, 0, 0))?;
        ...
        if table.rows.len() <= 1 {
            return Err(DocOpError::Invalid("Cannot delete the only row in a table".to_string()));
        }
        ...
        Ok(DocOp::InsertTableRow { table: table_idx, after_row })
    }
    
    /// Apply InsertTableColumn operation
    pub fn table_apply_insert_column(&mut self, table_idx: usize, after_col: usize) -> Result<DocOp, DocOpError> {
        // Validate table index
        let table_block = self.body.get_block_mut(table_idx)
            .ok_or_else(|| DocOpError::TableIndexOutOfRange(table_idx, 0, 0))?;
        ...
        if table.rows.is_empty() {
            return Err(DocOpError::Invalid("Cannot insert column into empty table".to_string()));
        }
        ...
        Ok(DocOp::DeleteTableColumn { table: table_idx, col: insert_col })
    }
    
    /// Apply DeleteTableColumn operation
    pub fn table_apply_delete_column(&mut self, table_idx: usize, col_idx: usize) -> Result<DocOp, DocOpError> {
        // Validate table index
        let table_block = self.body.get_block_mut(table_idx)
            .ok_or_else(|| DocOpError::TableIndexOutOfRange(table_idx, 0, 0))?;
        ...
        // Validate column index
        if table.rows.is_empty() {
            return Err(DocOpError::Invalid("Cannot delete column from empty table".to_string()));
        }
        ...
        if table.rows[0].cells.len() <= 1 {
            return Err(DocOpError::Invalid("Cannot delete the only column in a table".to_string()));
        }
        ...
        Ok(DocOp::InsertTableColumn { table: table_idx, after_col })
    }
    
    /// Apply MergeCells operation
    pub fn table_apply_merge_cells(&mut self, table_idx: usize, r1: usize, c1: usize, r2: usize, c2: usize) -> Result<DocOp, DocOpError> {
        // Validate table index
        let table_block = self.body.get_block_mut(table_idx)
            .ok_or_else(|| DocOpError::TableIndexOutOfRange(table_idx, 0, 0))?;
        ...
        let max_col = if max_row > 0 { table.rows[0].cells.len() } else { 0 };
        
        if r1 >= max_row || r2 >= max_row || c1 >= max_col || c2 >= max_col {
            return Err(DocOpError::TableIndexOutOfRange(table_idx, r1.max(r2), c1.max(c2)));
        }
        
        if r1 > r2 || c1 > c2 {
            return Err(DocOpError::OutOfRange("Merge range must have r1<=r2 and c1<=c2".to_string()));
        }
        ...
        if top_left_cell.row_span > 1 || top_left_cell.column_span > 1 {
            ...
            if top_left_cell.row_span == expected_row_span && top_left_cell.column_span == expected_col_span {
        ...
        Ok(DocOp::SplitCell { table: table_idx, row: start_row, col: start_col, horizontal: false })
    }
    
    /// Apply SplitCell operation
    pub fn table_apply_split_cell(&mut self, table_idx: usize, row: usize, col: usize, horizontal: bool) -> Result<DocOp, DocOpError> {
        // Validate table index
        let table_block = self.body.get_block_mut(table_idx)
            .ok_or_else(|| DocOpError::TableIndexOutOfRange(table_idx, 0, 0))?;
        ...
        let max_col = if table.rows[row].cells.len() > 0 { table.rows[row].cells.len() } else { 0 };
        if col >= max_col {
            return Err(DocOpError::TableIndexOutOfRange(table_idx, row, col));
        }
        ...
        if horizontal {
            ...
            Ok(DocOp::MergeCells { table: table_idx, r1: row, c1: col, r2, c2: col })
        } else {
            ...
            for c_offset in 1..col_span {
                ...
                for r_offset in 0..row_span {
                    ...
                    table.rows[target_row].cells.insert(insert_col, DocxTableCell {
                        paragraphs: vec![],
                        column_span: 1,
            ...
            Ok(DocOp::MergeCells { table: table_idx, r1: row, c1: col, r2: row + row_span - 1, c2 })
        }
    }
    
    /// Apply SetCellShading operation
    pub fn table_apply_set_cell_shading(&mut self, table_idx: usize, row: usize, col: usize, hex: String) -> Result<DocOp, DocOpError> {
        // Validate table index
        let table_block = self.body.get_block_mut(table_idx)
            .ok_or_else(|| DocOpError::TableIndexOutOfRange(table_idx, 0, 0))?;
...
mod tests {
    use super::*;
    use wo_ooxml::model::{DocxBody, DocxBlock, DocxParagraphProperties, DocxRun, DocxTable, DocxTableCell, DocxTableRow};
    
    fn create_test_table(rows: usize, cols: usize) -> DocxTable {
        DocxTable {
            rows: (0..rows).map(|_| DocxTableRow {
                cells: (0..cols).map(|_| DocxTableCell {
    ...
    fn test_insert_table_row() {
        ...
        let op = DocOp::InsertTableRow { table: 0, after_row: 0 };
    ...
    fn test_delete_table_row() {
        ...
        // Check inverse
        if let DocOp::InsertTableRow { table: t, after_row: r } = inverse {
            assert_eq!(t, 0);
            assert_eq!(r, 0);
        } else {
    ...
    fn test_insert_table_column() {
        ...
        let op = DocOp::InsertTableColumn { table: 0, after_col: 0 };
        let inverse = model.apply(&op).unwrap();
    ...
    fn test_delete_table_column() {
        ...
        // Check inverse
        if let DocOp::InsertTableColumn { table: t, after_col: c } = inverse {
            assert_eq!(t, 0);
            assert_eq!(c, 0);
        } else {
    ...
    fn test_merge_cells() {
        ...
        // Merge a 2x2 region
        let op = DocOp::MergeCells { table: 0, r1: 0, c1: 0, r2: 1, c2: 1 };
        let inverse = model.apply(&op).unwrap();
    ...
    fn test_split_cell_vertical() {
        ...
        let merge_op = DocOp::MergeCells { table: 0, r1: 0, c1: 0, r2: 0, c2: 2 };
        model.apply(&merge_op).unwrap();
        
        // Now split the first cell vertically
        let split_op = DocOp::SplitCell { table: 0, row: 0, col: 0, horizontal: false };
        ...
        match &body.blocks[0] {
            DocxBlock::Table(table) => {
                ...
                assert!(table.rows[0].cells.len() > 3, "Should have more than 3 cells after split");
    ...
    fn test_split_cell_horizontal() {
        ...
        let merge_op = DocOp::MergeCells { table: 0, r1: 0, c1: 0, r2: 2, c2: 0 };
        model.apply(&merge_op).unwrap();
        
        // Now split the first cell horizontally
        let split_op = DocOp::SplitCell { table: 0, row: 0, col: 0, horizontal: true };
        let result = model.apply(&split_op);
            ...
            DocxBlock::Table(table) => {
                ...
                for r in 0..3 {
                    if r < table.rows.len() && 0 < table.rows[r].cells.len() {
                        assert_eq!(table.rows[r].cells[0].row_span, 1, "Row {} col 0 should have row_span=1", r);
                    }
                }
            },
            _ => panic!("Expected table"),
    ...
    fn test_set_cell_shading() {
        ...
        // First set shading
        let op1 = DocOp::SetCellShading { table: 0, row: 0, col: 0, hex: old_shading.clone() };
        model.apply(&op1).unwrap();
        
        // Now change it
        let new_shading = "FF0000".to_string();
        let op2 = DocOp::SetCellShading { table: 0, row: 0, col: 0, hex: new_shading.clone() };
        ...
        // Check inverse
        if let DocOp::SetCellShading { table: _, row: _, col: _, hex: h } = inverse {
    ...
    fn test_merge_split_roundtrip() {
        ...
        // Merge cells
        let merge_op = DocOp::MergeCells { table: 0, r1: 0, c1: 0, r2: 1, c2: 1 };
        let _inverse_merge = model.apply(&merge_op).unwrap();
        
        // Split the merged cell vertically first
        let split_op1 = DocOp::SplitCell { table: 0, row: 0, col: 0, horizontal: false };
        model.apply(&split_op1).unwrap();
        
        // Split the first cell horizontally
        let split_op2 = DocOp::SplitCell { table: 0, row: 0, col: 0, horizontal: true };
                ...
                for row_idx in 0..2 {
                    for col_idx in 0..2 {
                        if row_idx < table.rows.len() && col_idx < table.rows[row_idx].cells.len() {
                            let cell = &table.rows[row_idx].cells[col_idx];
                            assert!(cell.row_span <= 1, "Cell at ({},{}) still has row_span > 1", row_idx, col_idx);
                            assert!(cell.column_span <= 1, "Cell at ({},{}) still has column_span > 1", row_idx, col_idx);
                            if row_idx == 0 && col_idx == 0 {
                                // The original content should be preserved
                                assert!(!cell.paragraphs.is_empty(), "Cell (0,0) should have content");
                            }
                        }
                    }
                }
    ...
    fn test_insert_row_at_end() {
        ...
        let op = DocOp::InsertTableRow { table: 0, after_row: 2 };
        let inverse = model.apply(&op).unwrap();
    ...
    fn test_delete_first_row() {
        ...
        if let DocOp::InsertTableRow { table: _, after_row: r } = inverse {
    ...
    fn test_cannot_split_unmerged_cell() {
        ...
        let op = DocOp::SplitCell { table: 0, row: 0, col: 0, horizontal: false };
        let result = model.apply(&op);
    ...
    fn test_merge_single_cell() {
        ...
        let op = DocOp::MergeCells { table: 0, r1: 0, c1: 0, r2: 0, c2: 0 };
        let result = model.apply(&op);
    ...
    fn test_insert_column_at_end() {
        ...
        let op = DocOp::InsertTableColumn { table: 0, after_col: 2 };
        let inverse = model.apply(&op).unwrap();
    ...
    fn test_delete_last_column() {
        ...
        if let DocOp::InsertTableColumn { table: _, after_col: c } = inverse {
            assert_eq!(c, 1);
        } else {
            panic!("Expected InsertTableColumn inverse");
        }
