...
fn calculate_inner_rect(chart: &Chart, rect: Rect) -> Rect {
    let mut inner = rect;
    if chart.title.as_ref().is_some_and(|t| t.visible) {
        inner.y += 40.0;
        inner.height -= 40.0;
    }
    let legend_height = if chart.legend.as_ref().is_some_and(|l| l.visible) {
        match chart.legend.as_ref().unwrap().position {
            LegendPosition::Top | LegendPosition::Bottom => 40.0,
            _ => 100.0,
        }
    } else {
        0.0
    ...
    inner.y += 20.0;
    inner.width -= 80.0;
    inner.height -= 40.0 + legend_height;
    inner
}
...
fn render_axes(chart: &Chart, canvas: &mut Canvas, rect: Rect) -> Result<(), ChartError> {
    ...
    for axis in &chart.axes {
        if !axis.show_labels && !axis.show_gridlines {
            continue;
        }
        ...
        match axis.position {
            AxisPosition::Bottom => {
                canvas.begin_path();
                canvas.move_to(rect.x, rect.bottom());
                canvas.line_to(rect.right(), rect.bottom());
                canvas.stroke();
            }
            AxisPosition::Left => {
                canvas.begin_path();
                canvas.move_to(rect.x, rect.y);
                canvas.line_to(rect.x, rect.bottom());
                canvas.stroke();
            }
            AxisPosition::Right => {
                canvas.begin_path();
                canvas.move_to(rect.right(), rect.y);
                canvas.line_to(rect.right(), rect.bottom());
                canvas.stroke();
            }
            AxisPosition::Top => {
                canvas.begin_path();
                canvas.move_to(rect.x, rect.y);
                canvas.line_to(rect.right(), rect.y);
                canvas.stroke();
            }
        }
    }
...
fn render_gridlines(chart: &Chart, canvas: &mut Canvas, rect: Rect) -> Result<(), ChartError> {
    ...
    if matches!(
        chart_kind,
        ChartKind::Bar | ChartKind::Column | ChartKind::Line | ChartKind::Area | ChartKind::Scatter
    ) {
        let num_lines = 5;
...
fn render_bar_chart(chart: &Chart, canvas: &mut Canvas, rect: Rect) -> Result<(), ChartError> {
    ...
    for (series_idx, series) in chart.series.iter().enumerate() {
        ...
        let bar_color = series
            ...
            .unwrap_or_else(|| default_series_color(series_idx));
        ...
        for (point_idx, point) in series.data.iter().enumerate() {
            ...
            canvas.begin_path();
            canvas.rect(bar_x, bar_y, bar_width.clamp(0.0, width), bar_height * 0.8);
            canvas.fill();
        }
...
fn render_column_chart(chart: &Chart, canvas: &mut Canvas, rect: Rect) -> Result<(), ChartError> {
    ...
    for (point_idx, _) in chart.series[0].data.iter().enumerate() {
        ...
        for (series_idx, series) in chart.series.iter().enumerate() {
            ...
            let point = match series.data.get(point_idx) {
                Some(p) => p,
                None => continue,
            };
            ...
            canvas.begin_path();
            canvas.rect(column_x, column_y, column_width * 0.8, column_height);
            canvas.fill();
        }
    }
    Ok(())
...
fn render_pie_chart(chart: &Chart, canvas: &mut Canvas, rect: Rect) -> Result<(), ChartError> {
    ...
    for (point_idx, point) in data_points.iter().enumerate() {
        ...
        let series_idx = 0;
        let slice_color = chart.series[series_idx]
            .color
            .as_ref()
            .and_then(|c| parse_hex_color(c))
            .unwrap_or_else(|| default_series_color(point_idx));
        canvas.set_fill(wo_renderer::color::Paint::Color(slice_color));
        canvas.begin_path();
        canvas.move_to(center_x, center_y);
        canvas.line_to(
            center_x + (current_angle.cos() * radius),
            center_y + (current_angle.sin() * radius),
        );
        let end_angle = current_angle + slice_angle;
        canvas.line_to(
            center_x + (end_angle.cos() * radius),
            center_y + (end_angle.sin() * radius),
        );
        canvas.close_path();
        canvas.fill();
        canvas.set_stroke(wo_renderer::color::Paint::Color(Color::WHITE));
...
fn render_radar_chart(chart: &Chart, canvas: &mut Canvas, rect: Rect) -> Result<(), ChartError> {
    ...
    for (series_idx, series) in chart.series.iter().enumerate() {
        ...
        for (point_idx, point) in series.data.iter().enumerate() {
            ...
            canvas.begin_path();
            canvas.circle(px, py, marker_radius);
            canvas.fill();
            canvas.set_stroke(wo_renderer::color::Paint::Color(Color::WHITE));
            canvas.set_stroke_style(white_style.clone());
            canvas.begin_path();
...
fn render_doughnut_chart(chart: &Chart, canvas: &mut Canvas, rect: Rect) -> Result<(), ChartError> {
    ...
    for (point_idx, point) in data_points.iter().enumerate() {
        ...
        canvas.begin_path();
        canvas.move_to(
            center_x + (current_angle.cos() * outer_radius),
            center_y + (current_angle.sin() * outer_radius),
        );
        let num_segments = 32;
        for i in 1..=num_segments {
            let angle = current_angle + (i as f32) * slice_angle / (num_segments as f32);
            canvas.line_to(
                center_x + (angle.cos() * outer_radius),
                center_y + (angle.sin() * outer_radius),
            );
        }
        for i in (0..=num_segments).rev() {
            let angle = current_angle + (i as f32) * slice_angle / (num_segments as f32);
            canvas.line_to(
                center_x + (angle.cos() * inner_radius),
                center_y + (angle.sin() * inner_radius),
            );
        }
        canvas.close_path();
        canvas.fill();
...
fn render_data_labels(chart: &Chart, _canvas: &mut Canvas, _rect: Rect) -> Result<(), ChartError> {
    if let Some(labels) = &chart.data_labels {
        if labels.visible {}
    }
    Ok(())
}
...
fn render_legend(chart: &Chart, canvas: &mut Canvas, rect: Rect) -> Result<(), ChartError> {
    if let Some(legend) = &chart.legend {
        ...
        let legend_x = match legend.position {
            LegendPosition::Left => rect.x + 10.0,
            LegendPosition::Right => rect.right() - 120.0,
            LegendPosition::Top | LegendPosition::TopRight => rect.x + (rect.width - 100.0),
            LegendPosition::Bottom => rect.x + (rect.width - 100.0) / 2.0,
        };
        let legend_y = match legend.position {
            LegendPosition::Top | LegendPosition::TopRight => rect.y + 20.0,
            LegendPosition::Bottom => rect.bottom() - 30.0,
            LegendPosition::Left | LegendPosition::Right => {
                rect.y + (rect.height - (chart.series.len() as f32 * 25.0)) / 2.0
            }
        };
        canvas.set_fill(wo_renderer::color::Paint::Color(Color::new(
            0.95, 0.95, 0.95, 0.9,
        ...
        let legend_height = (chart.series.len() as f32 * 25.0).max(25.0);
...
mod tests {
    ...
    mod render_bar_line_pie {
        ...
        #[test]
        fn render_bar_chart_with_multiple_series() {
            let mut chart = Chart::new(ChartKind::Bar);
            let mut series1 =
                Series::new("Series A", vec![DataPoint::new(10.0), DataPoint::new(20.0)]);
            series1.color = Some("#FF0000".to_string());
            chart.add_series(series1);
            let mut series2 =
                Series::new("Series B", vec![DataPoint::new(15.0), DataPoint::new(25.0)]);
            series2.color = Some("#00FF00".to_string());
        ...
        #[test]
        fn render_line_chart_with_multiple_series() {
            let mut chart = Chart::new(ChartKind::Line);
            let mut series1 = Series::new(
                "Series A",
                vec![
                    DataPoint::new(10.0),
                    DataPoint::new(20.0),
            ...
            series2.color = Some("#0000FF".to_string());
            chart.add_series(series2);
            let mut canvas = Canvas::new(800, 600);
            let rect = Rect::new(50.0, 50.0, 700.0, 500.0);
            assert!(render(&chart, &mut canvas, rect).is_ok());
        ...
        #[test]
        fn render_pie_chart_with_categories() {
            let mut chart = Chart::new(ChartKind::Pie);
            let mut series = Series::new(
                "Data",
                vec![
                    DataPoint::with_category(30.0, "Category A"),
                    DataPoint::with_category(40.0, "Category B"),
                    DataPoint::with_category(30.0, "Category C"),
                ],
            );
            series.color = Some("#FF00FF".to_string());
        ...
        #[test]
        fn render_chart_with_hidden_series() {
            let mut chart = Chart::new(ChartKind::Line);
            let mut visible_series =
                Series::new("Visible", vec![DataPoint::new(10.0), DataPoint::new(20.0)]);
            visible_series.visible = true;
            chart.add_series(visible_series);
            let mut hidden_series =
                Series::new("Hidden", vec![DataPoint::new(5.0), DataPoint::new(15.0)]);
            hidden_series.visible = false;
    ...
    mod render_scatter_area_radar {
        ...
        #[test]
        fn render_doughnut_chart_basic() {
        ...
        #[test]
        fn render_scatter_chart_with_multiple_series() {
            let mut chart = Chart::new(ChartKind::Scatter);
            let mut series1 = Series::new(
                "Series A",
                vec![
        ...
        #[test]
        fn render_area_chart_with_multiple_series() {
            let mut chart = Chart::new(ChartKind::Area);
            let mut series1 = Series::new(
                "Series A",
                vec![
        ...
        #[test]
        fn render_radar_chart_with_categories() {
            let mut chart = Chart::new(ChartKind::Radar);
            let mut series = Series::new(
                "Skills",
                vec![
            ...
            series.color = Some("#FF5733".to_string());
            chart.add_series(series);
            let mut canvas = Canvas::new(800, 600);
            let rect = Rect::new(50.0, 50.0, 700.0, 500.0);
            assert!(render(&chart, &mut canvas, rect).is_ok());
        ...
        #[test]
        fn render_doughnut_chart_with_categories() {
            let mut chart = Chart::new(ChartKind::Doughnut);
            let mut series = Series::new(
                "Expenses",
                vec![
            ...
            series.color = Some("#C70039".to_string());
            chart.add_series(series);
            let mut canvas = Canvas::new(800, 600);
            let rect = Rect::new(50.0, 50.0, 700.0, 500.0);
            assert!(render(&chart, &mut canvas, rect).is_ok());
