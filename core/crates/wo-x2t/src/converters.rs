...
fn txt_to_ooxml(txt_doc: &TxtDocument) -> OoxmlDocument {
    ...
    OoxmlDocument {
        ...
        relationships: vec![],
        xlsx_workbook: None,
        docx_body: Some(DocxBody::from_parts(paragraphs, vec![])),
    }
}
        ...
        match element {
            ...
            BlockElement::Div { elements, .. } | BlockElement::Blockquote { elements, .. } => {
                ...
                if let Some(body) = &sub.docx_body {
                    paragraphs.extend(body.paragraphs().into_iter().cloned());
                    tables.extend(body.tables().into_iter().cloned());
                }
            }
            BlockElement::RawHtml { content, .. } => {
                if !content.trim().is_empty() {
  ...
  <w:body>
    ...
    fn test_docx_runs_to_html_inlines_strikethrough() {
        ...
        match &inlines[0] {
            InlineElement::Strikethrough { content } => {
                assert!(
                    matches!(content[0], InlineElement::Text { text: ref t, style: None } if t == "struck")
                );
            }
    ...
    fn test_docx_runs_to_html_inlines_underline() {
        ...
        match &inlines[0] {
            InlineElement::Underline { content } => {
                assert!(
                    matches!(content[0], InlineElement::Text { text: ref t, style: None } if t == "under")
                );
            }
        ...
        match &inlines[0] {
            InlineElement::Bold { content } => match &content[0] {
                InlineElement::Italic { content: inner } => {
                    assert!(
                        matches!(inner[0], InlineElement::Text { text: ref t, style: None } if t == "both")
                    );
                }
