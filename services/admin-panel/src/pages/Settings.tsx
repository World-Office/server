            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid var(--wo-gray-200)', backgroundColor: 'var(--wo-gray-50)' }}>
                  <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontSize: '0.75rem', textTransform: 'uppercase', color: 'var(--wo-gray-500)' }}>Variable</th>
                  <th style={{ padding: '0.75rem 1rem', textAlign: 'left', fontSize: '0.75rem', textTransform: 'uppercase', color: 'var(--wo-gray-500)' }}>Value</th>
                </tr>
              </thead>
              <tbody>
                {envVars.map((env, index) => (
                  <tr key={env.key} style={{ borderBottom: index < envVars.length - 1 ? '1px solid var(--wo-gray-100)' : 'none' }}>
                    <td style={{ padding: '0.75rem 1rem', fontFamily: 'monospace', fontSize: '0.875rem' }}>{env.key}</td>
                    <td style={{ padding: '0.75rem 1rem', fontFamily: 'monospace', fontSize: '0.875rem', color: 'var(--wo-gray-500)' }}>{env.value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div>
        ...
        <div style={{ border: '1px solid var(--wo-gray-200)', borderRadius: 8, padding: '1.5rem', backgroundColor: 'white' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
            <div>
              <div style={{ fontSize: '0.75rem', color: 'var(--wo-gray-500)', marginBottom: '0.25rem' }}>Documents</div>
              <div style={{ fontSize: '1.25rem', fontWeight: 700 }}>0 MB</div>
            </div>
            <div>
              <div style={{ fontSize: '0.75rem', color: 'var(--wo-gray-500)', marginBottom: '0.25rem' }}>Users</div>
              <div style={{ fontSize: '1.25rem', fontWeight: 700 }}>0</div>
            </div>
            <div>
              <div style={{ fontSize: '0.75rem', color: 'var(--wo-gray-500)', marginBottom: '0.25rem' }}>Uptime</div>
