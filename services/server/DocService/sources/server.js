...
docsCoServer.install(server, app, () => {
  ...
  app.get(
    ...
    checkWopiEnable,
    utils.checkClientIp,
    wopiClient.collaboraCapabilities,
  )
  app.post(
    ...
    urleEcodedParser,
    fileForms.any(),
    converterService.convertTo,
  )
  app.post(
    "/cool/convert-to/:format?",
    checkWopiEnable,
    utils.checkClientIp,
    urleEcodedParser,
    fileForms.any(),
  ...
  app.get(
    "/hosting/wopi/convert-and-edit-handler",
    checkWopiEnable,
    utils.lowercaseQueryString,
    converterService.getConverterHtmlHandler,
  )
  app.get(
    "/wopi/files/:docid",
    apicache.middleware("5 minutes"),
    checkWopiDummyEnable,
    utils.lowercaseQueryString,
    wopiClient.dummyCheckFileInfo,
  )
  app.post("/wopi/files/:docid", checkWopiDummyEnable, wopiClient.dummyOk)
  app.get(
    "/wopi/files/:docid/contents",
    apicache.middleware("5 minutes"),
    checkWopiDummyEnable,
    wopiClient.dummyGetFile,
  )
  ...
  app.get("/plugins.json", (req, res) => {
    ...
    if (
      !config.has("services.CoAuthoring.server.static_content") ||
      !config.has("services.CoAuthoring.plugins.uri")
    ) {
      res.sendStatus(404)
      return
    }
  ...
  app.get("/themes.json", apicache.middleware("5 minutes"), (req, res) => {
    return co(function* () {
      ...
      try {
        ...
        if (
          !config.has("services.CoAuthoring.server.static_content") ||
          !config.has("services.CoAuthoring.themes.uri")
        ) {
          return
        }
        const staticContent = config.get("services.CoAuthoring.server.static_content")
  ...
  app.get("/document_editor_service_worker.js", apicache.middleware("5 min"), async (req, res) => {
    ...
    if (staticContent["/sdkjs"]) {
      ...
      res.sendFile(
        path.resolve(
          `${staticContent["/sdkjs"].path}/common/serviceworker/document_editor_service_worker.js`,
        ),
      )
    } else {
      res.sendStatus(404)
