...
function processConditionals(result, pruneFn) {
  ...
  if (pIf === null) result.if = undefined
  else result.if = pIf

  if (result.then) {
    const pThen = pruneFn(result.then)
    if (pThen === null) result.then = undefined
    else result.then = pThen
  }

  if (result.else) {
    const pElse = pruneFn(result.else)
    if (pElse === null) result.else = undefined
...
function deriveSchemaForScope(schema, scope) {
  const prune = (node) => {
    ...
    if (result.type === "object") {
      ...
      if (Array.isArray(result.required)) {
        result.required = result.required.filter(
          (k) => result.properties && Object.prototype.hasOwnProperty.call(result.properties, k),
        )
        if (result.required.length === 0) result.required = undefined
      }
    ...
    if (result.items) {
      ...
      if (prunedItems === null) result.items = undefined
